...
_instrument_providers: dict[str, Callable[[], object]] = {}
_pipeline_scope_providers: dict[str, Callable[[str], AbstractContextManager]] = {}
...
def register_pass_instrument_provider(name: str, provider: Callable[[], object]) -> None:
    """Register or replace a lazily-created PassContext instrument provider."""
    with _provider_lock:
        _instrument_providers[str(name)] = provider


def unregister_pass_instrument_provider(name: str) -> None:
    """Remove a previously registered instrument provider."""
    with _provider_lock:
        _instrument_providers.pop(str(name), None)


def create_registered_pass_instruments() -> list[object]:
    """Instantiate all instruments registered by active TileLang tools."""
    with _provider_lock:
        providers = tuple(_instrument_providers.values())
    return [provider() for provider in providers]


def register_pipeline_scope_provider(
    name: str,
    provider: Callable[[str], AbstractContextManager],
) -> None:
    """Register or replace a scope layered around each backend pipeline."""
    with _provider_lock:
        _pipeline_scope_providers[str(name)] = provider


def unregister_pipeline_scope_provider(name: str) -> None:
    """Remove a previously registered pipeline-scope provider."""
    with _provider_lock:
        _pipeline_scope_providers.pop(str(name), None)
...
def pass_pipeline(name: str) -> Generator[None, None, None]:
    ...
    with _provider_lock:
        providers = tuple(_pipeline_scope_providers.values())

    with pass_phase(base_phase), ExitStack() as stack:
        for provider in providers:
            stack.enter_context(provider(base_phase))
        yield
