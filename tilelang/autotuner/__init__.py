...
class AutoTuner:
    ...
    def run(self, warmup: int = 25, rep: int = 100, timeout: int = 30):
        ...
        ref_latency = None
        temp_jit_context = results_with_configs[0][0]
        if temp_jit_context.cache_input_tensors:
            supply_prog = temp_jit_context.supply_prog
            supply_type = temp_jit_context.supply_type
            profiler = temp_jit_context.kernel.get_profiler(tensor_supply_type=supply_type)
            jit_input_tensors_supply = get_input_tensors_supply(
                supply_prog, profiler, with_output=False)
