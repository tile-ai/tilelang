...
from tilelang.contrib.nvcc import get_target_compute_version, have_tma
...
def allow_tma_and_warp_specialized(pass_ctx: Optional[PassContext] = None,
    ...
    if not have_tma(get_target_compute_version(target.arch)):
        return False
...
def allow_fence_proxy(target: Optional[Target] = None) -> bool:
    return have_tma(get_target_compute_version(target.arch))
