...
GEMM_INST_METAL_SCALAR = "metal.scalar"
...
class GemmMetalScalar(GemmBase):
    ...
    def infer_layout(self, target: Target, thread_nums: int):
        ...
        layouts = {self.C: make_fully_replicated_layout_fragment(self.C, thread_nums)}
        if is_fragment(self.A):
            layouts[self.A] = make_fully_replicated_layout_fragment(self.A, thread_nums)
        if is_fragment(self.B):
            layouts[self.B] = make_fully_replicated_layout_fragment(self.B, thread_nums)
        return layouts
