...
def make_cutlass_metadata_layout_sm8x(buffer: tvm.tir.Buffer, mma_dtype: str):
    ...
    if mma_dtype in ["float8_e4m3", "float8_e5m2", "int8", "uint8"] and buffer.dtype not in [
            "uint32", "int32"]:
...
def make_cutlass_metadata_layout(
    buffer: tvm.tir.Buffer, mma_dtype: str = "float16", arch: str | None = None, **extra_args
):
    if arch is None:
