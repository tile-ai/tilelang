...
def make_swizzled_layout(
    buffer: Buffer | BufferLoad | BufferRegion, k_major: bool = True, allow_pad: bool = True
):
...
def make_volta_swizzled_layout(
    buffer: Buffer | BufferLoad | BufferRegion, is_a: bool = True, k_inner: bool = True
):
    stride, continuous = _get_stride_continuous(buffer)
...
def make_wgmma_swizzled_layout(
    buffer: Buffer | BufferLoad | BufferRegion, continuity: int = None, k_major: bool = True
):
    stride, continuous = _get_stride_continuous(buffer)
    element_size = _get_element_size(buffer)
