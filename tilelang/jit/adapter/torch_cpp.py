...
def torch_cpp_cuda_compile(code, target, verbose):
    ...
    name: str = "matmul"
    magic_number = 0x9f
    full_kernel_dir = TILELANG_JIT_WORKSPACE_DIR / Path(
        f"{name}_{magic_number}"
    )
    full_kernel_dir.mkdir(parents=True, exist_ok=True)

    sources: List[Union[str, Path]] = []

    tmp_cuda_kernel_file = (
        full_kernel_dir / "kernel.cu"
    )
...
class TorchCPPKernelAdapter(BaseKernelAdapter):
    ...
    def __init__(self, mod, params: List[TensorType], result_idx: List[int], target, prim_func, verbose: bool = False):
        self.target = target
