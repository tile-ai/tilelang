...
def ue4m3_scale_tile_source_coords(SF, block_mn: int, words_per_kblock: int, mn_block_idx, k_block_idx, word_flat):
    """Map a flat word index inside one NVFP4 scale tile to source coordinates.
    ...
    source tensor. This is the single place the tile addressing of the layout
    lives; kernels with custom staging schedules (e.g. producer warp groups)
    can build their own copy loops on top of it, and ``words_per_kblock`` is
    the knob for future contracts (block_K != 256, MX scale packings).
    """
    scale_cols = SF.shape[-1]
...
@T.macro
def copy_ue4m3_scale_tile(SF, SF_shared, mn_block_idx, k_block_idx):
    """Stage one NVFP4 scale tile from global into shared memory.

    NVFP4-specific: ``SF`` is the packed ``blockscaled_chunk_kmajor`` scale
    source produced by ``tilelang.quantize`` (``uint32 [MN, K/64]``, four
    UE4M3 scale bytes per word, ``sf_vec_size=16``); ``SF_shared`` is a
    ``uint32 [block_mn, block_K // 64]`` shared buffer consumed by
    ``T.mma_gemm_blockscaled``. Tiles are physically contiguous in this
    layout, so the copy is a cooperative whole-block transfer. Other
    block-scaled formats (MXFP4/MXFP8) will need their own staging helpers.
    """
    if len(SF_shared.shape) != 2 or len(SF.shape) != 2:
        raise ValueError(f"copy_ue4m3_scale_tile expects 2D scale buffers; got SF{tuple(SF.shape)} and SF_shared{tuple(SF_shared.shape)}")
    if str(SF.dtype) != "uint32" or str(SF_shared.dtype) != "uint32":
        raise ValueError(
            f"NVFP4 packed scale buffers must be uint32 words of four UE4M3 bytes; got SF={SF.dtype}, SF_shared={SF_shared.dtype}"
        )
    block_mn = int(SF_shared.shape[0])
    words_per_kblock = int(SF_shared.shape[1])
    scale_cols = int(SF.shape[1])
    if scale_cols % words_per_kblock != 0:
        raise ValueError(
            f"scale source has {scale_cols} K/64 words per row, which is not a "
            f"multiple of the shared tile's {words_per_kblock} words per k-block"
        )
    for word_flat in T.Parallel(block_mn * words_per_kblock):
        row, col = ue4m3_scale_tile_source_coords(SF, block_mn, words_per_kblock, mn_block_idx, k_block_idx, word_flat)
        SF_shared[word_flat // words_per_kblock, word_flat % words_per_kblock] = SF[row, col]
