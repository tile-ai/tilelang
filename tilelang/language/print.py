...
@macro
def print_flat_buffer_with_condition(condition:tir.PrimExpr, buffer: tir.Buffer, elems: int) -> tir.PrimExpr:
    if condition:
...
def print(obj: Any) -> tir.PrimExpr:
    ...
    if isinstance(obj, tir.Buffer):
        ...
        return print_flat_buffer_with_condition(
            condition, buffer, elems
        )
