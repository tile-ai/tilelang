...
def alloc_barrier(arrive_count: int):
    ...
    """
    return T.alloc_buffer([arrive_count], T.uint64, scope="shared.barrier")
...
def alloc_wgmma_desc(dtype: str = T.uint64):
    return alloc_descriptor("wgmma", dtype=dtype)


def alloc_tcgen05_smem_desc(dtype: str = T.uint64):
    return alloc_descriptor("tcgen05_smem", dtype=dtype)


def alloc_tcgen05_instruction_desc(dtype: str = T.uint32):
    return alloc_descriptor("tcgen05_instr", dtype=dtype)


# Alias: short name consistent with imports
def alloc_tcgen05_instr_desc(dtype: str = T.uint32):
    return alloc_tcgen05_instruction_desc(dtype)


@overload
def empty(shape: tuple[Unpack[_Shapes]], dtype: str = T.float32) -> Tensor[Callable[[Unpack[_Shapes]]], _DType]: ...


def empty(*shape: Unpack[_Shapes], dtype: str = T.float32) -> Tensor[Callable[[Unpack[_Shapes]]], _DType]:
