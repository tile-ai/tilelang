...
def view(
    ...
    If `shape` is None the source buffer's shape is used; if `dtype` is None the
    source buffer's dtype is used. The returned buffer shares the same underlying
    data as `src` (no copy). Explicit `strides` and `elem_offset` may be supplied
    to describe a non-row-major logical view.
    """
    if shape is None:
        shape = src.shape
    if dtype is None:
