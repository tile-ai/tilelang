    ...
    *,
    ...
    Args:
        ...
        remote_barrier (Optional[tir.BufferLoad], keyword-only): Shared-memory mbarrier element used for
            ...
            performed with a single bulk-async ``tl::tma_store_cluster`` call instead of the
            default element-by-element SIMT loop.  The barrier must be at the **same** shared
            memory offset in every CTA of the cluster (which is automatically true when all CTAs
            run the same kernel and declare the same shared-memory layout).  The destination CTA
            should wait on its local copy of this barrier after the copy completes.
    Raises:
