...
_LOAD_CACHE_POLICIES = frozenset({"ca", "cg", "cs", "lu", "cv"})
_STORE_CACHE_POLICIES = frozenset({"wb", "cg", "cs", "wt"})


def _set_cache_policy_annotation(
    annotations: dict,
    key: str,
    value: str | None,
    valid_policies: frozenset[str],
) -> None:
    """Set and validate one normal-copy CUDA cache-policy annotation."""
    if key not in annotations and value is not None:
        annotations[key] = tirx.StringImm(value)

    if key not in annotations:
        return

    policy = annotations[key]
    if not isinstance(policy, tirx.StringImm):
        raise TypeError(f"T.copy {key} must be a string, but got {type(policy).__name__}")
    if policy.value not in valid_policies:
        expected = ", ".join(f'"{name}"' for name in sorted(valid_policies))
        raise ValueError(f'T.copy {key}="{policy.value}" is invalid; expected one of: {expected}')
...
def copy(
    ...
    *,
    ...
    load_cache_policy: Literal["ca", "cg", "cs", "lu", "cv"] | None = None,
    store_cache_policy: Literal["wb", "cg", "cs", "wt"] | None = None,
    ...
    Args:
        ...
        eviction_policy (Optional[str], keyword-only): TMA cache eviction policy. Defaults to None.
        load_cache_policy (Optional[str], keyword-only): CUDA PTX cache operator for global
            loads in a normal SIMT copy. Supported values are ``"ca"``, ``"cg"``, ``"cs"``,
            ``"lu"``, and ``"cv"``. Requires ``prefer_instruction="sync"``.
        store_cache_policy (Optional[str], keyword-only): CUDA PTX cache operator for global
            stores in a normal SIMT copy. Supported values are ``"wb"``, ``"cg"``, ``"cs"``,
            and ``"wt"``. Requires ``prefer_instruction="sync"``.
        ...
        annotations (Optional[dict], keyword-only): Additional annotations dict. If provided,
            coalesced_width, disable_tma, eviction_policy, load_cache_policy,
            store_cache_policy, and prefer_instruction can also be specified here.
    ...
    Raises:
        TypeError: If a cache-policy annotation is not a string.
        ValueError: If a cache policy is not supported for its access direction.
    ...
    _set_cache_policy_annotation(ann, "load_cache_policy", load_cache_policy, _LOAD_CACHE_POLICIES)
    _set_cache_policy_annotation(ann, "store_cache_policy", store_cache_policy, _STORE_CACHE_POLICIES)
