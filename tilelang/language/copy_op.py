...
def tma_copy(
    ...
    *,
    ...
    Args:
        ...
        thread_extent: Number of threads to elect as TMA leader (e.g., 32 for per-warp).
            Defaults to the full block size if not specified.
    ...
    if thread_extent is not None:
        if not isinstance(thread_extent, int) or thread_extent <= 0:
            raise ValueError(f"thread_extent must be a positive int, got {thread_extent}")
        ann["thread_extent"] = thread_extent
