...
def do_bench(
    ...
    if fast_flush:
        ...
        cache = torch.empty(int(256e6), dtype=torch.int8, device="mps")
    ...
    if _n_repeat > 0:
        n_repeat = _n_repeat
    start_event = [torch.mps.Event(enable_timing=True) for _ in range(n_repeat)]
    end_event = [torch.mps.Event(enable_timing=True) for _ in range(n_repeat)]
    # Warm-up
