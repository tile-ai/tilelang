...
extern "C" __global__ void __launch_bounds__(256, 1) main_no_split_kernel(__grid_constant__ const CUtensorMap KV_desc, __grid_constant__ const CUtensorMap K_pe_desc, __grid_constant__ const CUtensorMap Output_desc, __grid_constant__ const CUtensorMap Q_desc, __grid_constant__ const CUtensorMap Q_pe_desc) {
  ...
  __shared__ uint64_t s_shared_ready_barrier[1];
  __shared__ uint64_t k_pe_shared_0_free_barrier[1];
  __shared__ uint64_t p0_1_1_ready_barrier[1];
  __shared__ uint64_t scale_1_ready_barrier[1];
  __shared__ uint64_t kv_shared_1_pe_is_ready[1];
  __shared__ uint64_t kv_shared_1_r_is_ready[1];
  __shared__ uint64_t lse_1_ready_barrier[1];
  __shared__ uint64_t lse_0_ready_barrier[1];
  __shared__ uint64_t score_max_0_ready_barrier[1];
  __shared__ uint64_t kv_shared_0_r_is_ready[1];
  __shared__ uint64_t s_shared_ready_barrier_1[1];
  __shared__ uint64_t k_pe_shared_1_free_barrier[1];
  __shared__ uint64_t kv_shared_0_l_is_ready[1];
  __shared__ uint64_t k_shared_1_l_free_barrier[1];
  __shared__ uint64_t q_shared_ready_barrier[1];
  __shared__ uint64_t kv_shared_1_l_is_ready[1];
  __shared__ uint64_t kv_shared_0_pe_is_ready[1];
  float acc_o_l[128];
  float logsum_0[2];
  float acc_s_0[32];
  ...
  if (((int)threadIdx.x) == 0) {
    tl::mbarrier_init(s_shared_ready_barrier[0], 128);
    tl::mbarrier_init(k_pe_shared_0_free_barrier[0], 128);
    tl::mbarrier_init(p0_1_1_ready_barrier[0], 128);
    tl::mbarrier_init(scale_1_ready_barrier[0], 128);
    tl::mbarrier_init(kv_shared_1_pe_is_ready[0], 128);
    tl::mbarrier_init(kv_shared_1_r_is_ready[0], 128);
    tl::mbarrier_init(lse_1_ready_barrier[0], 128);
    tl::mbarrier_init(lse_0_ready_barrier[0], 128);
    tl::mbarrier_init(score_max_0_ready_barrier[0], 128);
    tl::mbarrier_init(kv_shared_0_r_is_ready[0], 128);
    tl::mbarrier_init(s_shared_ready_barrier_1[0], 128);
    tl::mbarrier_init(k_pe_shared_1_free_barrier[0], 128);
    tl::mbarrier_init(kv_shared_0_l_is_ready[0], 128);
    tl::mbarrier_init(k_shared_1_l_free_barrier[0], 128);
    tl::mbarrier_init(q_shared_ready_barrier[0], 256);
    tl::mbarrier_init(kv_shared_1_l_is_ready[0], 128);
    tl::mbarrier_init(kv_shared_0_pe_is_ready[0], 128);
    tl::mbarrier_expect_tx(q_shared_ready_barrier[0], 32768);
    tl::tma_load(Q_desc, q_shared_ready_barrier[0], (&(((half_t*)buf_dyn_shmem)[47104])), 0, (((int)blockIdx.x) * 64), ((int)blockIdx.y));
    tl::tma_load(Q_desc, q_shared_ready_barrier[0], (&(((half_t*)buf_dyn_shmem)[51200])), 64, (((int)blockIdx.x) * 64), ((int)blockIdx.y));
    tl::tma_load(Q_desc, q_shared_ready_barrier[0], (&(((half_t*)buf_dyn_shmem)[55296])), 128, (((int)blockIdx.x) * 64), ((int)blockIdx.y));
    tl::tma_load(Q_desc, q_shared_ready_barrier[0], (&(((half_t*)buf_dyn_shmem)[59392])), 192, (((int)blockIdx.x) * 64), ((int)blockIdx.y));
    tl::mbarrier_expect_tx(q_shared_ready_barrier[0], 32768);
    tl::tma_load(Q_desc, q_shared_ready_barrier[0], (&(((half_t*)buf_dyn_shmem)[79872])), 256, (((int)blockIdx.x) * 64), ((int)blockIdx.y));
    tl::tma_load(Q_desc, q_shared_ready_barrier[0], (&(((half_t*)buf_dyn_shmem)[83968])), 320, (((int)blockIdx.x) * 64), ((int)blockIdx.y));
    tl::tma_load(Q_desc, q_shared_ready_barrier[0], (&(((half_t*)buf_dyn_shmem)[88064])), 384, (((int)blockIdx.x) * 64), ((int)blockIdx.y));
    tl::tma_load(Q_desc, q_shared_ready_barrier[0], (&(((half_t*)buf_dyn_shmem)[92160])), 448, (((int)blockIdx.x) * 64), ((int)blockIdx.y));
    tl::mbarrier_expect_tx(q_shared_ready_barrier[0], 8192);
    tl::tma_load(Q_pe_desc, q_shared_ready_barrier[0], (&(((half_t*)buf_dyn_shmem)[6144])), 0, (((int)blockIdx.x) * 64), ((int)blockIdx.y));
  }
  tl::mbarrier_arrive(q_shared_ready_barrier[0]);
  tl::mbarrier_wait(q_shared_ready_barrier[0], 0);
  ...
  if (((int)threadIdx.x) < 128) {
    ...
    if (((int)threadIdx.x) == 0) {
      tl::mbarrier_expect_tx(kv_shared_1_l_is_ready[0], 32768);
      tl::tma_load(KV_desc, kv_shared_1_l_is_ready[0], (&(((half_t*)buf_dyn_shmem)[14336])), 0, 0, 64, ((int)blockIdx.y));
      tl::tma_load(KV_desc, kv_shared_1_l_is_ready[0], (&(((half_t*)buf_dyn_shmem)[18432])), 64, 0, 64, ((int)blockIdx.y));
      tl::tma_load(KV_desc, kv_shared_1_l_is_ready[0], (&(((half_t*)buf_dyn_shmem)[22528])), 128, 0, 64, ((int)blockIdx.y));
      tl::tma_load(KV_desc, kv_shared_1_l_is_ready[0], (&(((half_t*)buf_dyn_shmem)[26624])), 192, 0, 64, ((int)blockIdx.y));
    }
    tl::mbarrier_arrive(kv_shared_1_l_is_ready[0]);
    if (((int)threadIdx.x) == 0) {
      tl::mbarrier_expect_tx(kv_shared_1_r_is_ready[0], 32768);
      tl::tma_load(KV_desc, kv_shared_1_r_is_ready[0], (&(((half_t*)buf_dyn_shmem)[30720])), 256, 0, 64, ((int)blockIdx.y));
      tl::tma_load(KV_desc, kv_shared_1_r_is_ready[0], (&(((half_t*)buf_dyn_shmem)[34816])), 320, 0, 64, ((int)blockIdx.y));
      tl::tma_load(KV_desc, kv_shared_1_r_is_ready[0], (&(((half_t*)buf_dyn_shmem)[38912])), 384, 0, 64, ((int)blockIdx.y));
      tl::tma_load(KV_desc, kv_shared_1_r_is_ready[0], (&(((half_t*)buf_dyn_shmem)[43008])), 448, 0, 64, ((int)blockIdx.y));
    }
    tl::mbarrier_arrive(kv_shared_1_r_is_ready[0]);
    if (((int)threadIdx.x) == 0) {
      tl::mbarrier_expect_tx(kv_shared_1_pe_is_ready[0], 8192);
      tl::tma_load(K_pe_desc, kv_shared_1_pe_is_ready[0], (&(((half_t*)buf_dyn_shmem)[2048])), 0, 0, 64, ((int)blockIdx.y));
    }
    tl::mbarrier_arrive(kv_shared_1_pe_is_ready[0]);
    for (int k = 0; k < 64; ++k) {
      tl::mbarrier_wait(kv_shared_0_l_is_ready[0], (k & 1));
      tl::gemm_ss<64, 64, 256, 4, 1, 0, 1, 1, true, -1>((&(((half_t*)buf_dyn_shmem)[47104])), (&(((half_t*)buf_dyn_shmem)[63488])), (&(acc_s_0[0])));
      tl::mbarrier_wait(kv_shared_0_r_is_ready[0], (k & 1));
      tl::gemm_ss<64, 64, 256, 4, 1, 0, 1, 0, true, -1>((&(((half_t*)buf_dyn_shmem)[79872])), (&(((half_t*)buf_dyn_shmem)[96256])), (&(acc_s_0[0])));
      tl::mbarrier_wait(kv_shared_0_pe_is_ready[0], (k & 1));
      ...
      tl::mbarrier_arrive(score_max_0_ready_barrier[0]);
      tl::mbarrier_wait(scale_1_ready_barrier[0], (k & 1));
      tl::__sync_thread_partial<3, 128>();
      if (k < 63) {
        if (((int)threadIdx.x) == 0) {
          tl::mbarrier_expect_tx(kv_shared_0_l_is_ready[0], 32768);
          tl::tma_load(KV_desc, kv_shared_0_l_is_ready[0], (&(((half_t*)buf_dyn_shmem)[63488])), 0, 0, ((k * 128) + 128), ((int)blockIdx.y));
          tl::tma_load(KV_desc, kv_shared_0_l_is_ready[0], (&(((half_t*)buf_dyn_shmem)[67584])), 64, 0, ((k * 128) + 128), ((int)blockIdx.y));
          tl::tma_load(KV_desc, kv_shared_0_l_is_ready[0], (&(((half_t*)buf_dyn_shmem)[71680])), 128, 0, ((k * 128) + 128), ((int)blockIdx.y));
          tl::tma_load(KV_desc, kv_shared_0_l_is_ready[0], (&(((half_t*)buf_dyn_shmem)[75776])), 192, 0, ((k * 128) + 128), ((int)blockIdx.y));
        }
        tl::mbarrier_arrive(kv_shared_0_l_is_ready[0]);
      }
      #pragma unroll
      ...
      tl::fence_proxy_async();
      tl::mbarrier_arrive(p0_1_1_ready_barrier[0]);
      #pragma unroll
      ...
      tl::fence_proxy_async();
      tl::mbarrier_wait(s_shared_ready_barrier[0], (k & 1));
      tl::gemm_ss<64, 256, 64, 4, 1, 0, 0, 0, true>((&(((half_t*)buf_dyn_shmem)[10240])), (&(((half_t*)buf_dyn_shmem)[14336])), (&(acc_o_l[0])));
      tl::mbarrier_arrive(k_pe_shared_0_free_barrier[0]);
      tl::mbarrier_arrive(k_shared_1_l_free_barrier[0]);
      tl::__sync_thread_partial<3, 128>();
      if (k < 63) {
        tl::mbarrier_wait(k_shared_1_l_free_barrier[0], (k & 1));
        if (((int)threadIdx.x) == 0) {
          tl::mbarrier_expect_tx(kv_shared_1_l_is_ready[0], 32768);
          tl::tma_load(KV_desc, kv_shared_1_l_is_ready[0], (&(((half_t*)buf_dyn_shmem)[14336])), 0, 0, ((k * 128) + 192), ((int)blockIdx.y));
          tl::tma_load(KV_desc, kv_shared_1_l_is_ready[0], (&(((half_t*)buf_dyn_shmem)[18432])), 64, 0, ((k * 128) + 192), ((int)blockIdx.y));
          tl::tma_load(KV_desc, kv_shared_1_l_is_ready[0], (&(((half_t*)buf_dyn_shmem)[22528])), 128, 0, ((k * 128) + 192), ((int)blockIdx.y));
          tl::tma_load(KV_desc, kv_shared_1_l_is_ready[0], (&(((half_t*)buf_dyn_shmem)[26624])), 192, 0, ((k * 128) + 192), ((int)blockIdx.y));
        }
        tl::mbarrier_arrive(kv_shared_1_l_is_ready[0]);
        tl::mbarrier_wait(k_pe_shared_1_free_barrier[0], (k & 1));
        if (((int)threadIdx.x) == 0) {
          tl::mbarrier_expect_tx(kv_shared_1_pe_is_ready[0], 8192);
          tl::tma_load(K_pe_desc, kv_shared_1_pe_is_ready[0], (&(((half_t*)buf_dyn_shmem)[2048])), 0, 0, ((k * 128) + 192), ((int)blockIdx.y));
        }
        tl::mbarrier_arrive(kv_shared_1_pe_is_ready[0]);
      }
    }
    ...
    tl::fence_proxy_async();
    tl::mbarrier_arrive(lse_0_ready_barrier[0]);
    tl::mbarrier_wait(lse_1_ready_barrier[0], 0);
    tl::__sync_thread_partial<3, 128>();
    #pragma unroll
    ...
    if (((int)threadIdx.x) == 128) {
      tl::mbarrier_expect_tx(kv_shared_0_l_is_ready[0], 32768);
      tl::tma_load(KV_desc, kv_shared_0_l_is_ready[0], (&(((half_t*)buf_dyn_shmem)[63488])), 0, 0, 0, ((int)blockIdx.y));
      tl::tma_load(KV_desc, kv_shared_0_l_is_ready[0], (&(((half_t*)buf_dyn_shmem)[67584])), 64, 0, 0, ((int)blockIdx.y));
      tl::tma_load(KV_desc, kv_shared_0_l_is_ready[0], (&(((half_t*)buf_dyn_shmem)[71680])), 128, 0, 0, ((int)blockIdx.y));
      tl::tma_load(KV_desc, kv_shared_0_l_is_ready[0], (&(((half_t*)buf_dyn_shmem)[75776])), 192, 0, 0, ((int)blockIdx.y));
    }
    tl::mbarrier_arrive(kv_shared_0_l_is_ready[0]);
    if (((int)threadIdx.x) == 128) {
      tl::mbarrier_expect_tx(kv_shared_0_r_is_ready[0], 32768);
      tl::tma_load(KV_desc, kv_shared_0_r_is_ready[0], (&(((half_t*)buf_dyn_shmem)[96256])), 256, 0, 0, ((int)blockIdx.y));
      tl::tma_load(KV_desc, kv_shared_0_r_is_ready[0], (&(((half_t*)buf_dyn_shmem)[100352])), 320, 0, 0, ((int)blockIdx.y));
      tl::tma_load(KV_desc, kv_shared_0_r_is_ready[0], (&(((half_t*)buf_dyn_shmem)[104448])), 384, 0, 0, ((int)blockIdx.y));
      tl::tma_load(KV_desc, kv_shared_0_r_is_ready[0], (&(((half_t*)buf_dyn_shmem)[108544])), 448, 0, 0, ((int)blockIdx.y));
    }
    tl::mbarrier_arrive(kv_shared_0_r_is_ready[0]);
    if (((int)threadIdx.x) == 128) {
      tl::mbarrier_expect_tx(kv_shared_0_pe_is_ready[0], 8192);
      tl::tma_load(K_pe_desc, kv_shared_0_pe_is_ready[0], (&(((half_t*)buf_dyn_shmem)[10240])), 0, 0, 0, ((int)blockIdx.y));
    }
    tl::mbarrier_arrive(kv_shared_0_pe_is_ready[0]);
    for (int k_1 = 0; k_1 < 64; ++k_1) {
      tl::mbarrier_wait(kv_shared_1_l_is_ready[0], (k_1 & 1));
      tl::gemm_ss<64, 64, 256, 4, 1, 0, 1, 1, true, -1>((&(((half_t*)buf_dyn_shmem)[47104])), (&(((half_t*)buf_dyn_shmem)[14336])), (&(acc_s_1[0])));
      tl::mbarrier_wait(kv_shared_1_r_is_ready[0], (k_1 & 1));
      tl::gemm_ss<64, 64, 256, 4, 1, 0, 1, 0, true, -1>((&(((half_t*)buf_dyn_shmem)[79872])), (&(((half_t*)buf_dyn_shmem)[30720])), (&(acc_s_1[0])));
      tl::mbarrier_wait(kv_shared_1_pe_is_ready[0], (k_1 & 1));
      tl::gemm_ss<64, 64, 64, 4, 1, 0, 1, 0, true, -1>((&(((half_t*)buf_dyn_shmem)[6144])), (&(((half_t*)buf_dyn_shmem)[2048])), (&(acc_s_1[0])));
      tl::wait_wgmma<0>();
      tl::mbarrier_wait(score_max_0_ready_barrier[0], (k_1 & 1));
      *(float4*)(scores_max_prev_1 + 0) = *(float4*)(((float*)buf_dyn_shmem) + ((((int)threadIdx.x) & 15) * 4));
      ...
      tl::fence_proxy_async();
      tl::mbarrier_arrive(scale_1_ready_barrier[0]);
      #pragma unroll
      ...
      tl::fence_proxy_async();
      tl::mbarrier_arrive(s_shared_ready_barrier[0]);
      tl::__sync_thread_partial<4, 128>();
      ...
      if (k_1 < 63) {
        if (((int)threadIdx.x) == 128) {
          tl::mbarrier_expect_tx(kv_shared_1_r_is_ready[0], 32768);
          tl::tma_load(KV_desc, kv_shared_1_r_is_ready[0], (&(((half_t*)buf_dyn_shmem)[30720])), 256, 0, ((k_1 * 128) + 192), ((int)blockIdx.y));
          tl::tma_load(KV_desc, kv_shared_1_r_is_ready[0], (&(((half_t*)buf_dyn_shmem)[34816])), 320, 0, ((k_1 * 128) + 192), ((int)blockIdx.y));
          tl::tma_load(KV_desc, kv_shared_1_r_is_ready[0], (&(((half_t*)buf_dyn_shmem)[38912])), 384, 0, ((k_1 * 128) + 192), ((int)blockIdx.y));
          tl::tma_load(KV_desc, kv_shared_1_r_is_ready[0], (&(((half_t*)buf_dyn_shmem)[43008])), 448, 0, ((k_1 * 128) + 192), ((int)blockIdx.y));
        }
        tl::mbarrier_arrive(kv_shared_1_r_is_ready[0]);
      }
      tl::mbarrier_wait(p0_1_1_ready_barrier[0], (k_1 & 1));
      tl::gemm_ss<64, 256, 64, 4, 1, 0, 0, 0, true>((&(((half_t*)buf_dyn_shmem)[2048])), (&(((half_t*)buf_dyn_shmem)[96256])), (&(acc_o_r[0])));
      tl::mbarrier_arrive(k_pe_shared_1_free_barrier[0]);
      tl::__sync_thread_partial<4, 128>();
      if (k_1 < 63) {
        if (((int)threadIdx.x) == 128) {
          tl::mbarrier_expect_tx(kv_shared_0_r_is_ready[0], 32768);
          tl::tma_load(KV_desc, kv_shared_0_r_is_ready[0], (&(((half_t*)buf_dyn_shmem)[96256])), 256, 0, ((k_1 * 128) + 128), ((int)blockIdx.y));
          tl::tma_load(KV_desc, kv_shared_0_r_is_ready[0], (&(((half_t*)buf_dyn_shmem)[100352])), 320, 0, ((k_1 * 128) + 128), ((int)blockIdx.y));
          tl::tma_load(KV_desc, kv_shared_0_r_is_ready[0], (&(((half_t*)buf_dyn_shmem)[104448])), 384, 0, ((k_1 * 128) + 128), ((int)blockIdx.y));
          tl::tma_load(KV_desc, kv_shared_0_r_is_ready[0], (&(((half_t*)buf_dyn_shmem)[108544])), 448, 0, ((k_1 * 128) + 128), ((int)blockIdx.y));
        }
        tl::mbarrier_arrive(kv_shared_0_r_is_ready[0]);
        tl::mbarrier_wait(k_pe_shared_0_free_barrier[0], (k_1 & 1));
        if (((int)threadIdx.x) == 128) {
          tl::mbarrier_expect_tx(kv_shared_0_pe_is_ready[0], 8192);
          tl::tma_load(K_pe_desc, kv_shared_0_pe_is_ready[0], (&(((half_t*)buf_dyn_shmem)[10240])), 0, 0, ((k_1 * 128) + 128), ((int)blockIdx.y));
        }
        tl::mbarrier_arrive(kv_shared_0_pe_is_ready[0]);
      }
    }
    tl::mbarrier_wait(lse_0_ready_barrier[0], 0);
    if ((((int)threadIdx.x) % 4) == 0) {
      #pragma unroll
    ...
    tl::fence_proxy_async();
    tl::mbarrier_arrive(lse_1_ready_barrier[0]);
    tl::__sync_thread_partial<4, 128>();
    #pragma unroll
