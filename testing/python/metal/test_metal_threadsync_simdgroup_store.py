...
def _staged_msg(x, Wg, Wu, Ws_g, mid, T_, Is, H, block_M=8, block_N=128, block_K=32, threads=256):
    ...
    Wu: T.Tensor((Is, H), "bfloat16")
    Ws_g: T.Tensor((1, H), "bfloat16")
    mid: T.Tensor((T_, Is), "bfloat16")
    with T.Kernel(T.ceildiv(T_, block_M), T.ceildiv(Is, block_N), threads=threads) as (bm, bn):
        ...
        Bg_s = T.alloc_shared((block_N, block_K), "bfloat16")
        Bu_s = T.alloc_shared((block_N, block_K), "bfloat16")
        Bs_s = T.alloc_shared((block_N, block_K), "bfloat16")
        ...
        for kk in T.serial(T.ceildiv(H, block_K)):
            for i, j in T.Parallel(block_M, block_K):
                A_s[i, j] = T.if_then_else(
                    (bm * block_M + i < T_) and (kk * block_K + j < H), x[bm * block_M + i, kk * block_K + j], T.cast(0, "bfloat16")
                )
            for i, j in T.Parallel(block_N, block_K):
                Bg_s[i, j] = T.if_then_else(
                    (bn * block_N + i < Is) and (kk * block_K + j < H), Wg[bn * block_N + i, kk * block_K + j], T.cast(0, "bfloat16")
                )
            for i, j in T.Parallel(block_N, block_K):
                Bu_s[i, j] = T.if_then_else(
                    (bn * block_N + i < Is) and (kk * block_K + j < H), Wu[bn * block_N + i, kk * block_K + j], T.cast(0, "bfloat16")
                )
            for i, j in T.Parallel(block_N, block_K):
                Bs_s[i, j] = T.if_then_else((i == 0) and (kk * block_K + j < H), Ws_g[0, kk * block_K + j], T.cast(0, "bfloat16"))
        ...
        for i, j in T.Parallel(block_M, block_N):
            if bm * block_M + i < T_ and bn * block_N + j < Is:
                ...
                mid[bm * block_M + i, bn * block_N + j] = T.cast((gg / (1 + T.exp(-gg))) * Cu_sh[i, j] * sig, "bfloat16")
...
def _dense_gemm_frag(A, B, C, M, N, K, block_M, block_N, block_K, threads):
    ...
    with T.Kernel(T.ceildiv(N, block_N), T.ceildiv(M, block_M), threads=threads) as (bx, by):
        A_s = T.alloc_shared((block_M, block_K), "bfloat16")
        B_s = T.alloc_shared((block_N, block_K), "bfloat16")
        C_l = T.alloc_fragment((block_M, block_N), "float32")
        T.clear(C_l)
        for kk in T.serial(T.ceildiv(K, block_K)):
            for i, j in T.Parallel(block_M, block_K):
                A_s[i, j] = T.if_then_else(
                    (by * block_M + i < M) and (kk * block_K + j < K), A[by * block_M + i, kk * block_K + j], T.cast(0, "bfloat16")
                )
            for i, j in T.Parallel(block_N, block_K):
                B_s[i, j] = T.if_then_else(
                    (bx * block_N + i < N) and (kk * block_K + j < K), B[bx * block_N + i, kk * block_K + j], T.cast(0, "bfloat16")
                )
...
def _staged_inputs(seed=11):
    ...
    x = (torch.randn(T_, H) * 0.5).to(torch.bfloat16).to(dev)
    Wg = (torch.randn(Is, H) * 0.05).to(torch.bfloat16).to(dev)
    Wu = (torch.randn(Is, H) * 0.05).to(torch.bfloat16).to(dev)
    Ws = (torch.randn(1, H) * 0.05).to(torch.bfloat16).to(dev)
    xf = x.float().cpu()
...
def _check_staged_threads(threads, n_rounds=N_ROUNDS, seed=11):
    """Run the staged repro with `threads` simdgroup threads and check the
    ...
    k = _staged_msg.compile(x, Wg, Wu, Ws, torch.zeros(T_, Is, dtype=torch.bfloat16, device=dev), T_, Is, H, 8, bn, 32, threads)
    facts = _msl_facts(k)
    fails = 0
    maxerr = 0.0
    det_ok = True
...
def test_codegen_staged_t256_barrier_after_store():
    ...
    k = _staged_msg.compile(x, Wg, Wu, Ws, torch.zeros(T_, Is, dtype=torch.bfloat16, device="mps"), T_, Is, H, 8, 128, 32, 256)
    facts = _msl_facts(k)
    assert facts["bar_after_store"], (
...
def test_correctness_staged_t32():
    ...
    (pre-fix baseline maxerr=2.4e-04 = bf16 noise floor)."""
...
def test_correctness_frag_t256_bit_identical():
    ...
    M, N, K = 64, 2048, 512
    torch.manual_seed(7)
    A = (torch.randn(M, K) * 0.5).to(torch.bfloat16).to(dev)
    B = (torch.randn(N, K) * 0.05).to(torch.bfloat16).to(dev)
    C = torch.empty(M, N, dtype=torch.float32, device=dev)
    _dense_gemm_frag(A, B, C, M, N, K, 8, 128, 32, 256)
    torch.mps.synchronize()
    ...
    for transpose, stride in AP_CONFIGS:
        for kind, kern, exp in (("raw", _access_ptr_raw, exp_raw), ("war", _access_ptr_war, exp_war)):
            ...
            for _ in range(N_ROUNDS):
                ...
                kern(x, out, transpose, stride, 64)
                torch.mps.synchronize()
                got = out.cpu()
                ref = exp.cpu()
