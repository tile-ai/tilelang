...
def test_lower_tmem_slice_copy_uses_slice_width():
    @T.prim_func
    def func(X: T.Tensor((256, 256), T.float16), Y: T.Tensor((256, 256), T.float16)):
        with T.Kernel(1, 1, threads=128) as (bx, by):
            A_shared = T.alloc_shared((128, 128), T.float16)
            ...
            C_tmem = T.alloc_tmem([128, 128], T.float32)
            mbar = T.alloc_barrier(1)
            C_local = T.alloc_fragment((128, 32), T.float32)
            T.copy(X[0, 0], A_shared)
            T.copy(X[0, 0], B_shared)
            T.tcgen05_gemm(
                A_shared,
    ...
    def visitor(node):
        if isinstance(node, tirx.Call) and hasattr(node, "op") and hasattr(node.op, "name") and node.op.name == "tl.tcgen05_ld":
            calls.append(node)

    tirx.stmt_functor.post_order_visit(mod["main"].body, visitor)
    assert len(calls) == 1
    assert calls[0].args[1].value == 32


def test_lower_tmem_copy_with_floating_dynamic_fragment_access():
    reduce_src = """
extern "C" __device__ float tl_test_read_first(float *ptr) {
  return ptr[0];
}
"""

    @T.prim_func
    def func(X: T.Tensor((256, 256), T.float16), Y: T.Tensor((1, 128), T.float32)):
        with T.Kernel(1, threads=384):
            T.import_source(reduce_src)
            A_shared = T.alloc_shared((128, 128), T.float16)
            B_shared = T.alloc_shared((128, 128), T.float16)
            C_tmem = T.alloc_tmem((128, 128), T.float32)
            ...
            if tx < 32:
                ...
                local_row = tx - 256
                T.mbarrier_wait_parity(mbar, 0)
                T.copy(C_tmem[:, 64:128], C_local)
                Y[0, local_row] = T.call_extern(
                    "float32",
                    "tl_test_read_first",
                    T.address_of(C_local[local_row, 0]),
                )
