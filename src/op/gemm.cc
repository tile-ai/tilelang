...
#define SUCCESS(atom_m, atom_n, atom_k)                                        \
  return {                                                                     \
    true, UTCMMAMeta { atom_m, atom_n, atom_k }                                \
  }
 ...
 */
 Stmt GemmNode::Lower(const LowerArgs &T, arith::Analyzer *analyzer) const {
  ...
  if (gemm_inst == GemmInst::kUTCMMA) {
    ...
    ICHECK(mbar.has_value()) << "mbar must be provided for UTCMMA";
    if (A.scope() == "shared.tmem") {
      op_name = "tl::utcmma_gemm_ts";
    } else if (A.scope() == "shared.dyn" || A.scope() == "shared") {
      op_name = "tl::utcmma_gemm_ss";
    } else {
      ICHECK(0)
          << "Unsupported A scope for UTCMMA: "
          << A.scope(); // If this is triggered, it means Tilelang has bugs.
    }
    ICHECK(wg_wait == -1)
        << "Currently only wg_wait == -1 is supported for UTCMMA. Please use "
    ...
    ICHECK(!accum_dtype.empty())
        << "Unsupported C dtype for UTCMMA: " << C->dtype;
    ss << op_name << "<" << M << ", " << N << ", " << K << ", ";
    ...
    // Since UTCMMA atoms provided by CUTLASS always have an internal
    ...
    ICHECK(
        ...
        analyzer->CanProveEqual(FloorMod(T.thread_bounds->extent, warp_size),
                                0))
        << "UTCMMA requires thread bounds to be multiples of warp size (32) "
  ...
  if (TargetIsHopper(T.target)) {
    ...
    // NOTE On sm100, only the leading thread issues the UTCMMA instruction but
    // all threads need to wait, so we emit another statement for cases where
    // wg_wait == 0.
