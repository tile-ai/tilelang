...
bool Gemm::CheckWGMMA() const {
  if (C->dtype == DataType::Float(16)) {
    if (A->dtype == DataType::Float(16) && B->dtype == DataType::Float(16))
      return K % 16 == 0;
    else if (A->dtype == DataType::NVFloat8E4M3() && B->dtype == DataType::NVFloat8E4M3())
      return (!trans_A) && trans_B && K % 32 == 0;
    else if (A->dtype == DataType::NVFloat8E4M3() && B->dtype == DataType::NVFloat8E5M2())
      return (!trans_A) && trans_B && K % 32 == 0;
    else if (A->dtype == DataType::NVFloat8E5M2() && B->dtype == DataType::NVFloat8E4M3())
      return (!trans_A) && trans_B && K % 32 == 0;
    else if (A->dtype == DataType::NVFloat8E5M2() && B->dtype == DataType::NVFloat8E5M2())
      return (!trans_A) && trans_B && K % 32 == 0;
    else
      return false;
  }
    ...
    if (A->dtype == DataType::Float(16) && B->dtype == DataType::Float(16))
      return K % 16 == 0;
    else if (A->dtype == DataType::BFloat(16) && B->dtype == DataType::BFloat(16))
      return K % 16 == 0;
...
LayoutMap Gemm::InferLayout(const LayoutInferArgs &T, InferLevel level) {
  ...
  if (TargetIsVolta(T.target)) {
    ...
    if (A.scope() == "shared" || A.scope() == "shared.dyn") {
      ...
      auto ABLayout = maybe_wgmma
                          ? makeGemmABLayoutHopper(mat_stride, mat_continuous,
                                                  continuity, A->dtype.bits(),
                                                  trans_A ? 1 : 2)
                          : makeGemmABLayout(mat_stride, mat_continuous,
    ...
    if (B.scope() == "shared" || B.scope() == "shared.dyn") {
      ...
      auto ABLayout = maybe_wgmma
                          ? makeGemmABLayoutHopper(mat_stride, mat_continuous,
                                                  continuity, B->dtype.bits(),
                                                  trans_B ? 2 : 1)
                          : makeGemmABLayout(mat_stride, mat_continuous,
                                             mat_continuous, B->dtype.bits(),
                                             trans_B ? 2 : 1);
