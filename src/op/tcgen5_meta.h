...
inline uint32_t GetTCGEN5MXF4NVF4BlockScaledInstrDesc(int atom_m, int atom_n,
  ...
  ICHECK(scale_in_a == 1 || scale_in_a == -1);
  ICHECK(scale_in_b == 1 || scale_in_b == -1);
  ICHECK(ab_dtype.is_float4_e2m1fn())
      << "ab_dtype must be float4_e2m1fn for mxf4nvf4";
  ...
  desc |= set_bits(0, 2, 1); // sparse_flag
  // bit 3 reserved
  // bit 4-5: b_sf_id, enforce to be 0 for scale_vec::4X
  // bit 6 reserved
  ...
  desc |= set_bits(n_dim, 17, 6); // n_dim
  // scale_format = 1 (E8M0) for MXFP4, 0 (E4M3) for NVFP4
  desc |= set_bits(is_mxfp4 ? 1 : 0, 23, 1);
  // bit 24-26 reserved
  desc |= set_bits(m_dim, 27, 2); // m_dim
  // bit 29-30: a_sf_id, enforce to be 0 for scale_vec::4X
  desc |= set_bits(0, 29, 2); // a_sf_id
  // bit 31 reserved
