...
private:
  ...
  struct OperandDriver {
    Buffer solved;
    bool pending{false};
  };
  OperandDriver FindOperandDriver(const std::vector<Buffer> &fragment_buffers,
  ...
  // as OPERAND-DRIVEN — adopt the layout induced by a solved driver operand
  // (FindOperandDriver), defer while a driver is still unsolved, and
  // self-plan only when no operand can drive the loop. Self-planning such a
