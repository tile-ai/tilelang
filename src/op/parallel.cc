...
LayoutMap ParallelOpNode::InferLayout(const LayoutInferArgs &T,
  ...
  auto compute_loop_layout_from_buffer = [&](const Buffer &buffer) {
    ...
    std::cerr << "[compute_loop_layout_from_buffer] infer from buffer `"
    ...
    if (IsCommonAccessIndice(buffer)) {
      ...
      PostOrderVisit(loop_var_to_thread, [&](const ObjectRef &objref) {
        if (auto opt_var = objref.as<Var>();
          ...
          std::cerr << "loop_var_to_thread = " << loop_var_to_thread
    ...
    std::cerr << "[compute_loop_layout_from_buffer] ... and get "
  ...
  if (source_buffer.defined()) {
    ...
    if (read_source_buffer.defined()) {
      ...
      std::cerr << "[PlanLoopPartition] vector_size = " << vector_size
                << std::endl;
      ...
      std::cerr << "[PlanLoopPartition] loop_total_size = " << loop_total_size
                << std::endl;
      while (!analyzer_.CanProve(
             ...
             vector_size > 1)
        vector_size /= 2;
      std::cerr << "[PlanLoopPartition] after adjust: vector_size = " << vector_size
                << std::endl;
