...
LayoutMap ParallelOp::InferLayout(const LayoutInferArgs &T, InferLevel level) {
  ...
  auto compute_loop_layout_from_buffer = [&](const Buffer &buffer) {
    ...
    if (IsCommonAccessIndice(buffer)) {
      ...
      return Fragment(loop_vars_, {}, loop_var_to_thread, rep_iter).SetThreadRange(T.thread_bounds);
    }
  };
  ...
  for (const auto &[buffer, _] : indice_map_) {
    if (!T.layout_map.count(buffer)) {
      results.Set(buffer, CompleteBufferFragment(buffer).SetThreadRange(T.thread_bounds));
    }
    ...
    if (buffer.scope() == "local.fragment" && source_buffer.defined() &&
      ...
      if (T.layout_map.count(buffer)) {
        ...
        Fragment dst_layout_fragment = CompleteBufferFragment(buffer).SetThreadRange(T.thread_bounds);
