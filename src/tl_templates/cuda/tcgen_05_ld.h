...
/**
 * Load N 32-bit words from a 32-bit source TMEM address into dst_ptr using
 * a 32-data-path, 32-bit pattern transfer (unpacked 16-bit lanes).
 *
 * N must be a power of two in the range 1..128; unsupported N triggers a trap.
 * The function populates dst_ptr[0..N-1] with values read from src_addr.
 *
 * @tparam N Number of repeated pattern transfers (power of two).
 * @param src_addr Source TMEM address encoded as a 32-bit value.
 * @param dst_ptr Destination buffer for N 32-bit words.
 */

/**
 * Load N 32-bit words from a 32-bit source TMEM address into dst_ptr using
 * a 32-data-path, 32-bit pattern transfer with packed 16-bit lanes.
 *
 * N must be a power of two in the range 1..128; unsupported N triggers a trap.
 * The function populates dst_ptr[0..N-1] with values read from src_addr.
 *
...
/**
 * Load N 32-bit words from a 32-bit source TMEM address into dst_ptr using
 * a 16-data-path, 64-bit pattern transfer (unpacked 16-bit lanes).
 *
 * N must be a power of two in the range 1..128; unsupported N triggers a trap.
 * The function populates dst_ptr[0..N-1] with values read from src_addr.
 *
...
/**
 * Load N 32-bit words from a 32-bit source TMEM address into dst_ptr using
 * a 16-data-path, 64-bit pattern transfer with packed 16-bit lanes.
 *
 * N must be a power of two in the range 1..128; unsupported N triggers a trap.
 * The function populates dst_ptr[0..N-1] with values read from src_addr.
 *
...
/**
 * Load N 32-bit words from a 32-bit source TMEM address into dst_ptr using
 * a 16-data-path, 128-bit pattern transfer (unpacked 16-bit lanes).
 *
 * N must be a power of two in the range 1..64; unsupported N triggers a trap.
 * For each repetition the load writes two 32-bit words per pattern, so
 * dst_ptr indices up to N*2-1 may be written.
 *
 * @tparam N Number of repeated pattern transfers (power of two).
 * @param src_addr Source TMEM address encoded as a 32-bit value.
 * @param dst_ptr Destination buffer for the loaded 32-bit words.
 */

/**
 * Load N 32-bit words from a 32-bit source TMEM address into dst_ptr using
 * a 16-data-path, 128-bit pattern transfer with packed 16-bit lanes.
 *
 * N must be a power of two in the range 1..64; unsupported N triggers a trap.
 * For each repetition the load writes two 32-bit words per pattern, so
 * dst_ptr indices up to N*2-1 may be written.
...
/**
 * Load N 32-bit words from a 32-bit source TMEM address into dst_ptr using
 * a 16-data-path, 256-bit pattern transfer (unpacked 16-bit lanes).
 *
 * N must be a power of two in the range 1..32; unsupported N triggers a trap.
 * Each repetition writes four 32-bit words, so dst_ptr indices up to N*4-1
 * may be written.
 *
 * @tparam N Number of repeated pattern transfers (power of two).
 * @param src_addr Source TMEM address encoded as a 32-bit value.
...
/**
 * Load N 32-bit words from a 32-bit source TMEM address into dst_ptr using
 * a 16-data-path, 256-bit pattern transfer with packed 16-bit lanes.
 *
 * N must be a power of two in the range 1..32; unsupported N triggers a trap.
 * Each repetition writes four 32-bit words, so dst_ptr indices up to N*4-1
 * may be written.
...
/**
 * Composite 32-data-path loader for 64-bit patterns that performs two
 * 16-data-path, 64-bit pattern loads to implement a 32-lane transfer.
 *
 * This delegates to the 16dp implementation twice:
 * - first for the lower 16 lanes at src_addr,
 * - then for the upper 16 lanes at src_addr + (16 << 16),
 * storing results contiguously into dst_ptr.
 *
 * @tparam N Number of repeated pattern transfers (power of two).
 * @param src_addr Source TMEM address encoded as a 32-bit value.
 * @param dst_ptr Destination buffer for the loaded 32-bit words.
...
/**
 * Composite 32-data-path loader for 128-bit patterns that performs two
 * 16-data-path, 128-bit pattern loads to implement a 32-lane transfer.
 *
 * This delegates to the 16dp implementation twice:
 * - first for the lower 16 lanes at src_addr,
 * - then for the upper 16 lanes at src_addr + (16 << 16),
 * storing results contiguously into dst_ptr (second call writes starting at dst_ptr + N*2).
 *
 * @tparam N Number of repeated pattern transfers (power of two).
 * @param src_addr Source TMEM address encoded as a 32-bit value.
 * @param dst_ptr Destination buffer for the loaded 32-bit words.
...
/**
 * Composite 32-data-path loader for 256-bit patterns that performs two
 * 16-data-path, 256-bit pattern loads to implement a 32-lane transfer.
 *
 * This delegates to the 16dp implementation twice:
 * - first for the lower 16 lanes at src_addr,
 * - then for the upper 16 lanes at src_addr + (16 << 16),
 * storing results contiguously into dst_ptr (second call writes starting at dst_ptr + N*4).
 *
 * @tparam N Number of repeated pattern transfers (power of two).
 * @param src_addr Source TMEM address encoded as a 32-bit value.
 * @param dst_ptr Destination buffer for the loaded 32-bit words.
