...
template <>
struct MmaBlockScaleConfig<MmaBlockScaleKind::kMxf4nvf4, 4, ScaleType::kUE4M3> {
...
TL_DEVICE void mma_block_scale_m16n8k64_mxf4nvf4_4x_ue4m3(
  ...
  asm volatile(
      ...
      : "r"(a[0]), "r"(a[1]), "r"(a[2]), "r"(a[3]), "r"(b[0]), "r"(b[1]),
        ...
        "h"(byte_id_a), "h"(thread_id_a), "r"(scale_b), "h"(byte_id_b),
        "h"(thread_id_b));
}
...
template <MmaBlockScaleKind Kind, int ScaleVecSize, ScaleType SType>
TL_DEVICE void mma_block_scale_sync(float *d, const uint32_t *a,
                                    const uint32_t *b, const float *c,
                                    uint32_t scale_a, uint16_t byte_id_a,
                                    uint16_t thread_id_a, uint32_t scale_b,
