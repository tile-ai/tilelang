...
template <class Impl> struct MmaBlockScaledImplTraits {
...
TL_DEVICE void call_fma_blockscaled_impl(
    typename MmaBlockScaledImplTraits<Impl>::DReg *d,
    const typename MmaBlockScaledImplTraits<Impl>::AReg *a,
    const typename MmaBlockScaledImplTraits<Impl>::BReg *b,
    const typename MmaBlockScaledImplTraits<Impl>::CReg *c,
    const typename MmaBlockScaledImplTraits<Impl>::SFAReg *sfa,
    const typename MmaBlockScaledImplTraits<Impl>::SFBReg *sfb, uint16_t id_a,
...
template <class Impl>
TL_DEVICE void
call_fma_blockscaled(typename MmaBlockScaledImplTraits<Impl>::DReg *d,
                     const typename MmaBlockScaledImplTraits<Impl>::AReg *a,
                     const typename MmaBlockScaledImplTraits<Impl>::BReg *b,
                     const typename MmaBlockScaledImplTraits<Impl>::CReg *c,
                     const typename MmaBlockScaledImplTraits<Impl>::SFAReg *sfa,
                     const typename MmaBlockScaledImplTraits<Impl>::SFBReg *sfb,
                     uint16_t id_a, uint16_t id_b) {
  call_fma_blockscaled_impl<Impl>(
      d, a, b, c, sfa, sfb, id_a, id_b,
      std::make_index_sequence<MmaBlockScaledImplTraits<Impl>::kDRegs>{},
      std::make_index_sequence<MmaBlockScaledImplTraits<Impl>::kARegs>{},
      std::make_index_sequence<MmaBlockScaledImplTraits<Impl>::kBRegs>{},
      std::make_index_sequence<MmaBlockScaledImplTraits<Impl>::kCRegs>{},
      std::make_index_sequence<MmaBlockScaledImplTraits<Impl>::kSFARegs>{},
      std::make_index_sequence<MmaBlockScaledImplTraits<Impl>::kSFBRegs>{});
}

#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 1200
#define TL_SM120_BLOCKSCALED_ASM(KindAsm, ScaleVecAsm, ShapeAsm, AAsm, BAsm,   \
                                 SFAsm, d0, d1, d2, d3, a0, a1, a2, a3, b0,    \
                                 b1, c0, c1, c2, c3, sfa, sfb, id_a, id_b)     \
...
#define TL_DEFINE_SM120_BLOCKSCALED_IMPL(                                      \
  ...
  struct ImplName {                                                            \
    ...
    fma(float &d0, float &d1, float &d2, float &d3, uint32_t const &a0,        \
        ...
        uint16_t id_b) {                                                       \
      TL_SM120_BLOCKSCALED_ASM(KindAsm, ScaleVecAsm, ShapeAsm, AAsm, BAsm,     \
...
#define TL_SM120_MXF8F6F4_IMPL_NAME(ATypeEnum, BTypeEnum)                      \
  TL_SM120_MXF8F6F4_IMPL_NAME_EXPANDED(TL_BLOCKSCALED_DTYPE_ASM(ATypeEnum),    \
                                       TL_BLOCKSCALED_DTYPE_ASM(BTypeEnum))

#define TL_SM120_MXF4NVF4_IMPL_NAME_(ScaleName)                                \
  SM120_16x8x64_F32_e2m1_e2m1_F32_##ScaleName##_TN

#define TL_BLOCKSCALED_DTYPE_ASM_kFloat4_e2m1fn e2m1
#define TL_BLOCKSCALED_DTYPE_ASM_kFloat6_e2m3fn e2m3
#define TL_BLOCKSCALED_DTYPE_ASM_kFloat6_e3m2fn e3m2
#define TL_BLOCKSCALED_DTYPE_ASM_kFloat8_e4m3 e4m3
#define TL_BLOCKSCALED_DTYPE_ASM_kFloat8_e5m2 e5m2

#define TL_BLOCKSCALED_DTYPE_ASM_(TypeEnum) TL_BLOCKSCALED_DTYPE_ASM_##TypeEnum
#define TL_BLOCKSCALED_DTYPE_ASM(TypeEnum) TL_BLOCKSCALED_DTYPE_ASM_(TypeEnum)

#define TL_SM120_BLOCKSCALED_DTYPE_ROWS(M)                                     \
  M(kFloat4_e2m1fn)                                                            \
  M(kFloat6_e2m3fn)                                                            \
  M(kFloat6_e3m2fn)                                                            \
  M(kFloat8_e4m3)                                                              \
...
#define TL_SM120_BLOCKSCALED_DTYPE_COLS(M, ATypeEnum)                          \
...
#define TL_BLOCKSCALED_STRINGIZE_(X) #X
#define TL_BLOCKSCALED_STRINGIZE(X) TL_BLOCKSCALED_STRINGIZE_(X)

#define TL_DEFINE_SM120_MXF8F6F4_IMPL(ATypeEnum, BTypeEnum)                    \
  TL_DEFINE_SM120_BLOCKSCALED_IMPL(                                            \
      TL_SM120_MXF8F6F4_IMPL_NAME(ATypeEnum, BTypeEnum), "mxf8f6f4", "1X",     \
      "m16n8k32",                                                              \
      TL_BLOCKSCALED_STRINGIZE(TL_BLOCKSCALED_DTYPE_ASM(ATypeEnum)),           \
      TL_BLOCKSCALED_STRINGIZE(TL_BLOCKSCALED_DTYPE_ASM(BTypeEnum)), "ue8m0",  \
      uint8_t)                                                                 \
  TL_DEFINE_MMA_BLOCKSCALED_DISPATCHER(                                        \
...
#define TL_DEFINE_SM120_MXF4NVF4_IMPL(ScaleVecValue, ScaleVecAsm, ScaleName)   \
  TL_DEFINE_SM120_BLOCKSCALED_IMPL(                                            \
      TL_SM120_MXF4NVF4_IMPL_NAME_(ScaleName), "mxf4nvf4", ScaleVecAsm,        \
      "m16n8k64", "e2m1", "e2m1", TL_BLOCKSCALED_STRINGIZE(ScaleName),         \
      uint32_t)                                                                \
  TL_DEFINE_MMA_BLOCKSCALED_DISPATCHER(                                        \
...
struct MmaBlockScaledDispatcher {
  using CRegType = void;
  using ARegType = void;
  using BRegType = void;
...
#define TL_DEFINE_MMA_BLOCKSCALED_DISPATCHER(                                  \
  ...
  struct MmaBlockScaledDispatcher<DataType::ATypeEnum, DataType::BTypeEnum,    \
    ...
    static TL_DEVICE void exec(CRegType *d, const ARegType *a,                 \
                               ...
                               uint16_t id_a, uint16_t id_b) {                 \
      call_fma_blockscaled<Impl>(d, a, b, c, sfa, sfb, id_a, id_b);            \
    }                                                                          \
  };

#define TL_DEFINE_SM120_MXF8F6F4_ROW(ATypeEnum)                                \
  TL_SM120_BLOCKSCALED_DTYPE_COLS(TL_DEFINE_SM120_MXF8F6F4_IMPL, ATypeEnum)

// SM120 dense block-scaled MMA, mxf8f6f4 scale_vec::1X, ue8m0 scale factors.
TL_SM120_BLOCKSCALED_DTYPE_ROWS(TL_DEFINE_SM120_MXF8F6F4_ROW)
...
#undef TL_DEFINE_MMA_BLOCKSCALED_DISPATCHER
...
#undef TL_BLOCKSCALED_STRINGIZE
#undef TL_BLOCKSCALED_STRINGIZE_
#undef TL_SM120_BLOCKSCALED_DTYPE_COLS
#undef TL_SM120_BLOCKSCALED_DTYPE_ROWS
#undef TL_BLOCKSCALED_DTYPE_ASM
#undef TL_BLOCKSCALED_DTYPE_ASM_
#undef TL_BLOCKSCALED_DTYPE_ASM_kFloat8_e5m2
#undef TL_BLOCKSCALED_DTYPE_ASM_kFloat8_e4m3
#undef TL_BLOCKSCALED_DTYPE_ASM_kFloat6_e3m2fn
#undef TL_BLOCKSCALED_DTYPE_ASM_kFloat6_e2m3fn
#undef TL_BLOCKSCALED_DTYPE_ASM_kFloat4_e2m1fn
#undef TL_DEFINE_SM120_BLOCKSCALED_IMPL
#undef TL_SM120_BLOCKSCALED_ASM
...
TL_DEVICE void mma_sync_blockscaled(
    typename detail::MmaBlockScaledDispatcher<
        AType, BType, CType, M, N, K, TransA, TransB, ScaleVec>::CRegType *c,
    const typename detail::MmaBlockScaledDispatcher<
    ...
    uint16_t id_a, uint16_t id_b) {
  using Dispatcher =
      detail::MmaBlockScaledDispatcher<AType, BType, CType, M, N, K, TransA,
                                       TransB, ScaleVec>;
