...
// ldg32: Load 32 bits, return uint32_t
template <typename T> TL_DEVICE uint32_t ldg32(const T *ptr) {
  uint32_t ret{};
  global_load<uint32_t, 4>(ret, ptr, true);
  return ret;
}

// ldg64: Load 64 bits, return uint64_t
template <typename T> TL_DEVICE uint2 ldg64(const T *ptr) {
  uint2 ret{};
  global_load<uint2, 8>(ret, ptr, true);
  return ret;
}

// ldg128: Load 128 bits, return uint4
template <typename T> TL_DEVICE uint4 ldg128(const T *ptr) {
  uint4 ret{};
...
// Convenience wrapper functions for direct use
// stg32: Store 32 bits
template <typename T> TL_DEVICE void stg32(T *ptr, uint32_t value) {
  global_store<uint32_t, 4>(ptr, value, true);
}

// stg64: Store 64 bits
template <typename T> TL_DEVICE void stg64(T *ptr, uint2 value) {
  global_store<uint2, 8>(ptr, value, true);
}

// stg128: Store 128 bits
template <typename T> TL_DEVICE void stg128(T *ptr, uint4 value) {
