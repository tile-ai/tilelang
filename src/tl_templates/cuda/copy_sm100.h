...
__device__ __forceinline__ longlong4 ldg256(const longlong4 *ptr) {
  longlong4 ret{};
  global_load_256<longlong4>(ret, ptr, true);
  return ret;
}

__device__ __forceinline__ ulonglong4 ldg256(const ulonglong4 *ptr) {
  ulonglong4 ret{};
...
template <typename T>
__device__ __forceinline__ ulonglong4 ldg256(const T *ptr) {
  ulonglong4 ret{};
  global_load_256<ulonglong4>(ret, ptr, true);
  return ret;
}
...
__device__ __forceinline__ void stg256(longlong4 *ptr, const longlong4 &val) {
  global_store_256<longlong4>(val, ptr, true);
}
...
template <typename T> __device__ __forceinline__ void stg256(T *ptr, T &val) {
