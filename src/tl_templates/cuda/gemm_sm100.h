...
// About why we need .ws, plz refer to comments in tl_umma::GemmTensorOp
...
TL_DEVICE void utcmma_gemm_ss(A_type *pA, B_type *pB, uint32_t accum,
                              uint64_t *umma_bar_ptr, bool clear_accum) {
  using MMA = cute::tl_umma::GemmTensorOp<M, N, K, AtomM, AtomN, AtomK, trans_A,
