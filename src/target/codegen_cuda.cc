...
void CodeGenTileLangCUDA::VisitExpr_(const CallNode *op, std::ostream &os) {
  ...
  if (op->op.same_as(builtin::ptx_cp_async())) {
    ...
    if (op->args.size() == 1) {
      this->PrintIndent();
      auto mbarrier_obj = print_mbarrier_obj(op->args[0]);
      this->stream << "tl::mbarrier_arrive(" << mbarrier_obj << ");\n";
  ...
  } else if (op->op.same_as(tl::get_cluster_block_rank())) {
    ICHECK_EQ(op->args.size(), 0)
        << "tl.get_cluster_block_rank expects no arguments.";
    this->need_cooperative_groups_ = true;
    os << "cooperative_groups::this_cluster().block_rank()";
