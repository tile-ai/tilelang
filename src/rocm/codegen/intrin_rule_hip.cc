...
struct HIPMath {
  std::string operator()(DataType t, std::string name) const {
    // For vector types (e.g. float32x4 generated by T.Parallel when
    // BLOCK_M > threads), dispatch using the scalar element type so
    // that intrinsics like exp2f are emitted correctly inside the
    // scalar loop of PrintVecBinaryOp.
    if (!t.is_scalar()) {
      return (*this)(t.element_of(), name);
    }
    if (t.is_float()) {
      switch (t.bits()) {
      case 64:
