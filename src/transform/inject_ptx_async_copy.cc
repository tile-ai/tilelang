      ...
      if (bytes == 4 || bytes == 8 || bytes == 16) {
        ...
        if (indices_lanes == 1) {
          ...
          auto dst_access_ptr = store->buffer.access_ptr(
              2, DataType::Handle(), 1, mul(dst_offset, PrimExpr(index_factor)),
        ...
        if (!predicated) {
          ...
          // Predicated vectorized cp.async not supported with new access_ptr
          // API
          LOG(WARNING) << "Predicated vectorized cp.async not yet supported "
                          "with access_ptr API, "
