...
struct TileLangThreadSyncPlanner : public ConstrVisitor {
  ...
  struct AccessEntry {
     ...
     * same-index reasoning for ordering (AS-10: multi-simdgroup staged
     * epilogues got 0 barriers between simdgroup_store and cross-group
     * reads).
     */
  ...
  void VisitExpr_(const CallNode *op) final {
    ...
    if (op->op.same_as(builtin::simdgroup_store()) ||
      ...
      // epilogues with 0 barriers between the staging stores and the
      // cross-group epilogue reads (AS-10: t256 staged 12/12 rounds wrong;
      // static MSL had all barriers in the kk loop). Record the pointer arg
      // explicitly as a tile access with the full tile extent and the
      // correct access type instead.
      //
    ...
    if (op->op.same_as(builtin::address_of())) {
      ...
      if (Enabled(buffer_var, scope)) {
        ...
        // could prove it disjoint from other accesses via the single-element
        // PointerAccessIsDisjoint reasoning -- the AS-10 class of missed
        // barriers, for the Metal-macro/tensor-intrin simdgroup path
...
private:
  ...
  bool FindConflict(const AccessEntry &prev, const AccessEntry &curr,
    ...
    // below cannot prove safety for these (AS-10: multi-simdgroup staged
