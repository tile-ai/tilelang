...
class TileLangThreadSyncPlanner : public TileLangStorageAccessVisitor {
 public:
  explicit TileLangThreadSyncPlanner(StorageScope sync_scope) : sync_scope_(sync_scope) {}
 ...
 public:
  ...
  Stmt VisitStmt_(const AttrStmtNode* op) final {
    if (op->attr_key == tvm::tir::attr::async_wait_queue_scope) {
      ...
      ICHECK(inner && inner->attr_key == tvm::tir::attr::async_wait_inflight_count);
      auto zero = make_zero(DataType::Int(32));
      auto new_body = SeqStmt({sync, inner->body});
      return AttrStmt(zero, tvm::tir::attr::async_wait_queue_scope, op->value,
                      AttrStmt(zero, tvm::tir::attr::async_wait_inflight_count, inner->value, new_body));
    }
