...
bool IsSharedLikeScope(const String &scope) {
  return scope == "shared" || scope == "shared.dyn";
}

bool IsSharedLikeBuffer(const Buffer &buffer) {
  StorageScope scope = StorageScope::Create(GetPtrStorageScope(buffer->data));
  return scope.rank == StorageRank::kShared &&
         (scope.tag.empty() || scope.tag == "dyn");
}

bool IsSharedLikePointerVar(const VarNode *var) {
  if (var == nullptr) {
    return false;
  }
  ...
  return IsSharedLikeScope(ptr_type->storage_scope);
}

bool ExprContainsSharedLikePointerVar(const PrimExpr &expr) {
  bool has_shared_ptr = false;
  PostOrderVisit(expr, [&](const ObjectRef &node) {
    if (has_shared_ptr) {
      return;
    }
    if (const auto *var = node.as<VarNode>()) {
      has_shared_ptr = IsSharedLikePointerVar(var);
    }
  });
  return has_shared_ptr;
}

bool IsCPAsyncCall(const CallNode *call) {
  return call && (call->op.same_as(builtin::ptx_cp_async()) ||
                  call->op.same_as(tl::ptx_cp_async()));
}

bool IsCommitCall(const CallNode *call) {
  return call && call->op.same_as(builtin::ptx_commit_group());
}

bool IsWaitCall(const CallNode *call) {
  return call && call->op.same_as(builtin::ptx_wait_group());
}

bool IsFenceProxyAsyncCall(const CallNode *call) {
  return call && call->op.same_as(tl::fence_proxy_async());
}

bool CallMayTouchSharedMemory(const CallNode *call) {
  if (call == nullptr) {
    return false;
  }
  if (call->op.same_as(builtin::tvm_storage_sync())) {
  ...
  if (call->op.same_as(builtin::tvm_access_ptr())) {
    return call->args.size() >= 2 &&
           ExprContainsSharedLikePointerVar(call->args[1]);
  }
  if (call->op.same_as(builtin::address_of())) {
    ...
    return load != nullptr && IsSharedLikeBuffer(load->buffer);
  }
  for (const PrimExpr &arg : call->args) {
    if (ExprContainsSharedLikePointerVar(arg)) {
      return true;
    }
  }
  return false;
...
class BarrierTransparentStmtAnalyzer : public StmtExprVisitor {
public:
  bool Analyze(const Stmt &stmt) {
    transparent_ = true;
    VisitStmt(stmt);
    return transparent_;
  }
...
private:
  ...
  void VisitStmt_(const IfThenElseNode *op) final { transparent_ = false; }

  void VisitStmt_(const BufferStoreNode *op) final {
    if (IsSharedLikeBuffer(op->buffer)) {
      transparent_ = false;
      return;
    }
  ...
  void VisitExpr_(const CallNode *op) final {
    if (IsCPAsyncCall(op) || IsCommitCall(op) || IsWaitCall(op) ||
        IsFenceProxyAsyncCall(op)) {
      return;
    }
    if (CallMayTouchSharedMemory(op)) {
      transparent_ = false;
      return;
    }
    StmtExprVisitor::VisitExpr_(op);
...
class ThreadStorageSyncOptimizer : public StmtExprMutator {
public:
  Stmt VisitStmt_(const SeqStmtNode *op) final {
    Array<Stmt> visited;
    visited.reserve(op->seq.size());
    ...
    std::vector<bool> remove(visited.size(), false);
    for (int i = 0, n = static_cast<int>(visited.size()); i < n; ++i) {
      if (!IsSharedStorageSyncStmt(visited[i])) {
        continue;
      }
      for (int j = i + 1; j < n; ++j) {
        if (IsSharedStorageSyncStmt(visited[j])) {
          break;
        }
        if (IsWaitGroupStmt(visited[j]) && j + 1 < n &&
            IsSharedStorageSyncStmt(visited[j + 1])) {
          remove[i] = true;
          break;
        }
        if (IsLoopEntryCoveredByWaitAndSync(visited[j])) {
          remove[i] = true;
          break;
        }
        if (!IsBarrierTransparentStmt(visited[j])) {
          break;
        }
      }
    }
...
private:
  bool IsBarrierTransparentStmt(const Stmt &stmt) const {
    BarrierTransparentStmtAnalyzer analyzer;
    return analyzer.Analyze(stmt);
  }

  bool IsSharedStorageSyncStmt(const Stmt &stmt) const {
    return GetSharedStorageSyncScope(stmt).has_value();
  }

  std::optional<String> GetSharedStorageSyncScope(const Stmt &stmt) const {
    if (const auto *let = stmt.as<LetStmtNode>()) {
      return GetSharedStorageSyncScope(let->body);
    }
    if (const auto *attr = stmt.as<AttrStmtNode>()) {
      return GetSharedStorageSyncScope(attr->body);
    }
    if (const auto *seq = stmt.as<SeqStmtNode>()) {
      if (seq->seq.size() == 1) {
        return GetSharedStorageSyncScope(seq->seq[0]);
      }
      return std::nullopt;
    }
    if (const auto *block = stmt.as<BlockNode>()) {
      return GetSharedStorageSyncScope(block->body);
    }
    if (const auto *realize = stmt.as<BlockRealizeNode>()) {
      if (is_one(realize->predicate)) {
        return GetSharedStorageSyncScope(realize->block->body);
      }
      return std::nullopt;
    }
    ...
    const auto *scope = call->args[0].as<StringImmNode>();
    if (scope == nullptr || !IsSharedLikeScope(scope->value)) {
      return std::nullopt;
    }
    return scope->value;
  }
  ...
  bool IsWaitGroupStmt(const Stmt &stmt) const {
    ...
    const auto *call = eval->value.as<CallNode>();
    return IsWaitCall(call);
  }

  const ForNode *GetSimpleForNode(const Stmt &stmt) const {
    if (const auto *let = stmt.as<LetStmtNode>()) {
      return GetSimpleForNode(let->body);
    }
    if (const auto *attr = stmt.as<AttrStmtNode>()) {
      return GetSimpleForNode(attr->body);
    }
    if (const auto *seq = stmt.as<SeqStmtNode>()) {
      if (seq->seq.size() == 1) {
        return GetSimpleForNode(seq->seq[0]);
      }
      return nullptr;
    }
    if (const auto *block = stmt.as<BlockNode>()) {
      return GetSimpleForNode(block->body);
    }
    if (const auto *realize = stmt.as<BlockRealizeNode>()) {
      if (is_one(realize->predicate)) {
        return GetSimpleForNode(realize->block->body);
      }
      return nullptr;
    }
    return stmt.as<ForNode>();
  }

  std::vector<Stmt> GetSimpleStmtList(const Stmt &stmt) const {
    if (const auto *let = stmt.as<LetStmtNode>()) {
      return GetSimpleStmtList(let->body);
    }
    if (const auto *attr = stmt.as<AttrStmtNode>()) {
      return GetSimpleStmtList(attr->body);
    }
    if (const auto *seq = stmt.as<SeqStmtNode>()) {
      std::vector<Stmt> out;
    ...
    if (const auto *block = stmt.as<BlockNode>()) {
      return GetSimpleStmtList(block->body);
    }
    if (const auto *realize = stmt.as<BlockRealizeNode>()) {
      if (is_one(realize->predicate)) {
        return GetSimpleStmtList(realize->block->body);
      }
      return {stmt};
    }
    return {stmt};
  ...
  bool IsProvablyPositiveExtent(const PrimExpr &extent) const {
    if (const auto *imm = extent.as<IntImmNode>()) {
      return imm->value > 0;
    }
    arith::Analyzer analyzer;
    return analyzer.CanProve(extent > 0);
  }

  bool IsLoopEntryCoveredByWaitAndSync(const Stmt &stmt) const {
    const auto *loop = GetSimpleForNode(stmt);
    if (loop == nullptr || !IsProvablyPositiveExtent(loop->extent)) {
      return false;
    }

    std::vector<Stmt> body_stmts = GetSimpleStmtList(loop->body);
    for (size_t i = 0; i < body_stmts.size(); ++i) {
      if (IsWaitGroupStmt(body_stmts[i])) {
        return i + 1 < body_stmts.size() &&
               IsSharedStorageSyncStmt(body_stmts[i + 1]);
      }
      if (!IsBarrierTransparentStmt(body_stmts[i])) {
        return false;
      }
    }
    return false;
...
PrimFunc TileLangThreadSync(PrimFunc func, const std::string &storage_scope) {
  ...
  stmt = ThreadPartialSyncRewriter::Rewrite(std::move(stmt));
  if (sync_scope.rank == StorageRank::kShared) {
    stmt = ThreadStorageSyncOptimizer()(std::move(stmt));
  }
  n->body = std::move(stmt);
  return func;
}
