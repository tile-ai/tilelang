...
/**
 * @brief Construct a ReducerInfoNode from textual op and replication descriptors.
 *
 ...
 * @throws RuntimeError if op_str or rep_str is not one of the supported values (triggers ICHECK).
 */
...
private:
  /**
   * @brief Visit an attribute statement and capture the IterVar for threadIdx.x.
   *
   * If the attribute key is `tir::attr::thread_extent` and the node is an `IterVar`
   * whose `thread_tag` equals `"threadIdx.x"`, this sets the mutator's `thread_var_`
   * to that IterVar (after asserting the iterator's extent is an `IntImm`). The
   * previous `thread_var_` is preserved and restored after delegating to the base
   * visitor. Delegates all traversal work to `IRMutatorWithAnalyzer::VisitStmt_`.
   *
   * Side effects:
   * - Temporarily updates the member `thread_var_` during traversal of the child
   *   statement so subsequent visitors can read the thread index IterVar.
   *
  ...
  /**
   * @brief Visits a TIR Block node to collect reducer metadata and apply discovered buffer layouts.
   *
   * This method:
   * - Extracts reducer information from the block's `attr::kReducerInfo` annotation and
   *   populates the internal reducer_info_map_.
   * - Registers allocated buffers by mapping each buffer's data Var to its Buffer in var_to_buffer_.
   * - Recursively visits and rewrites the block body via the base mutator.
   * - Merges any layouts accumulated in new_layout_map_ into the block's `attr::kLayoutMap` annotation
   *   (creating or extending the annotation), then clears new_layout_map_ for subsequent blocks.
   *
   * Side effects:
   * - Updates reducer_info_map_, var_to_buffer_, and may set the block-level `kLayoutMap` annotation.
   ...
   * inside a reduction region (tracked by inside_reducer_range_) and this is the
   * outermost loop of that region, annotates the loop with reducer information
   * and derives per-buffer layout fragments for each reducer buffer.
   *
   * When annotating:
   ...
   *   - If the reducer's replication type is ALL, creates a replication fragment
   *     across the thread extent.
   ...
   * - Each reducer Var in inside_reducer_range_ must map to an allocated Buffer in
   *   var_to_buffer_ (ICHECK enforced).
   *
   ...
   * implemented.
   *
   * @return Stmt The (possibly transformed) statement returned by the base mutator.
   */
  ...
  /**
   * @brief Processes Call expressions to track reducer ranges and finalize reducer operations.
   *
   * Visits call nodes, detects T.fill calls that target reducer buffers and records their
   * reducer metadata in inside_reducer_range_ until the matching T.finalize_reducer is seen.
   * When a FinalizeReducerOp call is encountered, this method appends the reducer operation
   * enum value to the call arguments and removes the corresponding entry from inside_reducer_range_.
   *
   * Side effects:
   * - Inserts and removes entries in inside_reducer_range_.
   * - Mutates the FinalizeReducerOp call by pushing the reducer op enum as an extra argument.
   *
   * Failure modes:
   * - ICHECK fails if a T.fill targets a reducer already recorded in inside_reducer_range_
   *   (i.e., a prior T.fill without an intervening T.finalize_reducer).
   * - ICHECK fails if T.finalize_reducer has no matching T.fill (no entry in inside_reducer_range_).
   *
  ...
  /**
       ...
       * arith::Analyzer, which the mutator uses to query symbolic bounds and simplify
       * integer expressions during layout inference.
       *
...
public:
   ...
   *
   * Runs the ReducerLayoutAnnotator over the function body to collect reducer metadata,
   * insert layout mappings for reducer buffers, and lower local.reducer usage to
   * local.fragment-compatible forms. Returns a new PrimFunc whose body is the transformed IR.
   *
   * @param f The PrimFunc to transform; passed by value and returned with an updated body.
   * @return PrimFunc The transformed PrimFunc with reducer layouts and related rewrites applied.
   */
...
/**
 * @brief Create a TVM transform pass that lowers local.reducer buffers to local.fragment layouts.
 *
 * This pass runs ReducerLayoutAnnotator::Substitute on a PrimFunc to collect reducer metadata,
 * compute per-buffer layout fragments for reducer buffers, and annotate blocks with the
 * resulting layout map. It is exposed as a PrimFunc-level pass named "tl.LayoutReducer".
 *
 * @return tvm::transform::Pass A prim-function pass that applies the layout-reduction substitution.
 */
