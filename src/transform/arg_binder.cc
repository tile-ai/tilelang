 ...
 */
 ...
 void ArgBinder::BindDLTensor(const Buffer &buffer, const PrimExpr &device_type,
  ...
  // Shape 指针本身用 if_then_else 做 NULL guard，避免在 handle 为 NULL 时
  // 直接 TVMStructGet(handle, kArrShape) 解引用。
  init_nest_.emplace_back(
  ...
  for (size_t k = 0; k < buffer->shape.size(); ++k) {
    // 这些 packed-bit dtype 的 shape 在原始实现里就没有绑定，直接沿用。
    ...
    // 从 DLTensor 里读出来的“真·运行时 shape 值”
    ...
    // 绑定到 TIR 里符号维度（比如 m）的值，加一层 is_null guard：
    //   handle 为 NULL → 用 0，占个位但不解引用
    //   handle 非 NULL → 真正从 DLTensor 的 shape 数组里读
    PrimExpr bound_shape_val = tvm::if_then_else(
        is_null,
        make_zero(buffer->shape[k].dtype()),
        raw_shape_val);

    // 第一次见到 Var（例如 m）时，这里会生成
    //   Let(m, bound_shape_val, ...)
    // 常数维度则只会生成约束断言。
    Bind_(buffer->shape[k], bound_shape_val, shape_element_name(k), true);

    // 保留一个显式的“一致性检查”：非 NULL 时，符号维度必须等于 DLTensor 的 shape。
    Stmt shape_check =
        AssertStmt(Or(is_null, buffer->shape[k] == raw_shape_val),
  ...
  if (const auto* const_offset = buffer->elem_offset.as<IntImmNode>()) {
    // 常量 elem_offset：只需要做一致性检查，不需要额外绑定 Var。
