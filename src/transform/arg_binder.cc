...
void ArgBinder::BindDLTensor(const Buffer &buffer, const PrimExpr &device_type,
  ...
  for (size_t k = 0; k < buffer->shape.size(); ++k) {
    // Skip shape validation for bitpacked / bool buffers, consistent with
    // original version.
    ...
    // The k-th dimension of runtime DLTensor (only actually loaded when handle
    // is non-NULL)
    ...
    // Bind to the value of this dimension expression in TIR (could be a Var,
    // e.g., m/n):
    //   handle non-NULL: raw_shape_val
    //   handle is NULL: 0 (placeholder to avoid "undefined variable", but won't
    //   force it to equal 0)
    PrimExpr bind_value = tvm::if_then_else(
        Not(is_null), raw_shape_val, make_zero(buffer->shape[k].dtype()));
    ...
    if (vdim != nullptr) {
      ...
      if (it == def_map_->end()) {
        ...
        // Manually simulate Bind_'s "define Var + Let binding" logic, but don't
        // use Bind_ itself, so we can control the subsequent constraint form.
        ...
        // This Var has already been bound on another Buffer (e.g., A / C),
        // reuse directly.
        lhs = it->second;
      }
    } else {
      // Constant extent (e.g., 768), only need consistency check, no binding
      // required.
      lhs = buffer->shape[k];
    }
    ...
    PrimExpr cond = tvm::if_then_else(Not(is_null), eq, const_true(1));
  ...
  if (const auto *const_offset = buffer->elem_offset.as<IntImmNode>()) {
    ...
    Bind_(buffer->elem_offset, bound_elem_off, arg_name + ".elem_offset", true);
