 ...
 */
 ...
 void ArgBinder::BindDLTensor(const Buffer &buffer, const PrimExpr &device_type,
  ...
  // Still use if_then_else here to avoid direct struct_get when handle is NULL.
  init_nest_.emplace_back(
  ...
  for (size_t k = 0; k < buffer->shape.size(); ++k) {
    // Skip shape validation for bitpacked / bool buffers, consistent with original version.
    ...
    // The k-th dimension of runtime DLTensor (only actually loaded when handle is non-NULL)
    ...
    // Bind to the value of this dimension expression in TIR (could be a Var, e.g., m/n):
    //   handle non-NULL: raw_shape_val
    //   handle is NULL: 0 (placeholder to avoid "undefined variable", but won't force it to equal 0)
    PrimExpr bind_value = tvm::if_then_else(
        Not(is_null),
        raw_shape_val,
        make_zero(buffer->shape[k].dtype()));

    const VarNode* vdim = buffer->shape[k].as<VarNode>();
    PrimExpr lhs;

    if (vdim != nullptr) {
      auto it = def_map_->find(vdim);
      if (it == def_map_->end()) {
        // First time seeing this dimension variable (e.g., m / n):
        // Manually simulate Bind_'s "define Var + Let binding" logic, but don't use Bind_ itself,
        // so we can control the subsequent constraint form.
        Var v_arg = Downcast<Var>(buffer->shape[k]);
        defs_.emplace_back(v_arg);
        (*def_map_)[vdim] = v_arg;
        init_nest_.emplace_back(LetStmt(v_arg, bind_value, nop));
        lhs = v_arg;
      } else {
        // This Var has already been bound on another Buffer (e.g., A / C), reuse directly.
        lhs = it->second;
      }
    } else {
      // Constant extent (e.g., 768), only need consistency check, no binding required.
      lhs = buffer->shape[k];
    }

    // Now add shape consistency constraint:
    //
    //   - when handle is non-NULL: lhs == raw_shape_val
    //   - when handle is NULL: constraint is always True ("don't check")
    //
    // Note: this is if_then_else(Not(is_null), lhs == raw_shape_val, True)
    // not lhs == if_then_else(...)
    PrimExpr eq = (lhs == raw_shape_val);
    PrimExpr cond =
  ...
  if (buffer->strides.empty()) {
    ...
    for (size_t i = buffer->shape.size(); i != 0; --i) {
      size_t k = i - 1;
      PrimExpr svalue =
          cast(stype, BufferLoad(buf_strides,
                                 {IntImm(DataType::Int(32),
                                         static_cast<int>(k))}));
    ...
    if (!conds.empty()) {
      ...
      Stmt check =
          AssertStmt(foldl([](PrimExpr a, PrimExpr b, Span span) {
                              return logical_and(a, b, span);
                            },
                           const_true(1), conds),
    ...
    for (size_t i = buffer->shape.size(); i != 0; --i) {
      ...
      core_value = tvm::if_then_else(buffer->shape[k] == 1,
                                     make_zero(stride_dtype),
                                     core_value);

      // Bind NULL-safe version to buffer->strides[k]
      PrimExpr bound_stride_val = tvm::if_then_else(
          is_null,
          make_zero(stride_dtype),
          core_value);
      ...
      Stmt stride_check =
          AssertStmt(Or(is_null, buffer->strides[k] == core_value),
                     StringImm(stride_element_name(k) +
                               " mismatch with DLTensor strides"),
                     Evaluate(0));
  ...
  if (const auto* const_offset = buffer->elem_offset.as<IntImmNode>()) {
    // Constant elem_offset: only need consistency check, no need for additional Var binding.
    ...
    PrimExpr bound_elem_off = tvm::if_then_else(
        is_null,
        make_zero(buffer->elem_offset.dtype()),
        expect_elem_off);
    Bind_(buffer->elem_offset, bound_elem_off,
