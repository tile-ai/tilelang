...
private:
  void VisitStmt_(const ForNode *node) final {
    inner_for_ = node;
    LOG(INFO) << "inner_for_->thread_binding :" << inner_for_->thread_binding;
...
//       Substitute(expr, {{var, v1 * target_vectorized_size}}));
//   LOG(INFO) << "expr_transformed " << expr_transformed;
...
//   LOG(INFO) << "expr_vectorized " << expr_vectorized;
  ...
  Stmt VisitStmt_(const ForNode *node) final {
    ...
    if (inner_for_ == node) { // rewrite the innermost loop
      ...
      if (!dynamic_) {
        ...
        vmap.Set(tx_var, truncmod(tx_var, 32) * vector_size_);
        vmap.Set(fnode->loop_var,
                 outer_var * vector_size_ + truncdiv(tx_var, 32));
