...
// Compatibility wrappers for ICHECK/CHECK from older TVM API.
// New tvm-ffi uses TVM_FFI_ICHECK(cond) and TVM_FFI_CHECK(cond, err_kind).
// These wrappers default to InternalError / ValueError respectively.
#ifndef ICHECK
#define ICHECK(cond) TVM_FFI_ICHECK(cond)
#endif
#ifndef ICHECK_EQ
#define ICHECK_EQ(a, b) TVM_FFI_ICHECK_EQ(a, b)
#endif
#ifndef ICHECK_GT
#define ICHECK_GT(a, b) TVM_FFI_ICHECK_GT(a, b)
#endif
#ifndef ICHECK_GE
#define ICHECK_GE(a, b) TVM_FFI_ICHECK_GE(a, b)
#endif
#ifndef ICHECK_LT
#define ICHECK_LT(a, b) TVM_FFI_ICHECK_LT(a, b)
#endif
#ifndef ICHECK_LE
#define ICHECK_LE(a, b) TVM_FFI_ICHECK_LE(a, b)
#endif
#ifndef CHECK
#define CHECK(cond) TVM_FFI_CHECK(cond, ValueError)
#endif
#ifndef CHECK_EQ
#define CHECK_EQ(a, b) TVM_FFI_CHECK_EQ(a, b, ValueError)
#endif
 ...
 * var_remap. Will throw errors if there is a mismatch.
 */
class SymbolicMatcher
    : ExprFunctor<void(const PrimExpr &n, const PrimExpr &other)> {
public:
  explicit SymbolicMatcher(arith::Analyzer *analyzer,
                           ffi::Map<tirx::Var, PrimExpr> *var_remap)
      : analyzer_(analyzer), var_remap_(var_remap) {}

  void Match(const ffi::Array<PrimExpr> &params,
...
private:
  void VisitExpr(const PrimExpr &node, const PrimExpr &other) {
    if (node.same_as(other)) {
      ...
      LOG(FATAL) << "Parameter expression " << node << " with dtype "
                 << node.dtype() << " cannot match to argument " << other
                 << " with dtype " << other.dtype();
    } else {
...
#define TVM_DECLARE_SYMBOLIC_MATCHER_BINOP(OpName)                             \
  ...
  void VisitExpr_(const VarNode *op, const PrimExpr &rhs) {
    ...
    if (lhs.same_as(rhs)) {
      ...
      LOG(FATAL) << "Parameter expression " << ffi::GetRef<PrimExpr>(op)
                 << " with dtype " << op->dtype << " cannot match to argument "
                 << rhs << " with dtype " << rhs.dtype();
...
private:
  PrimExpr VisitExpr_(const VarNode *_op) final {
    if (auto it = var_remap_.find(ffi::GetRef<Var>(_op));
        it != var_remap_.end()) {
      return (*it).second;
    } else {
  ...
  Stmt VisitStmt_(const SBlockNode *_op) final {
    SBlock sblock = Downcast<SBlock>(StmtMutator::VisitStmt_(_op));

    // Define the mutation functions.

    auto f_mutate_match_buffers = [this](
    ...
    auto f_mutate_read_write_region =
    ...
    ffi::Array<MatchBufferRegion> match_buffers =
        MutateArray(sblock->match_buffers, f_mutate_match_buffers);
    // Step 2. Mutate the read/write region.
    ffi::Array<BufferRegion> reads =
        MutateArray(sblock->reads, f_mutate_read_write_region);
    ffi::Array<BufferRegion> writes =
        MutateArray(sblock->writes, f_mutate_read_write_region);
    // Step 3. Mutate the Allocate Buffers.
    ffi::Array<Buffer> alloc_buffers =
        MutateArray(sblock->alloc_buffers, [this](const Buffer &buffer) {
    ...
    if (reads.same_as(sblock->reads) &&   //
        writes.same_as(sblock->writes) && //
        match_buffers.same_as(sblock->match_buffers) &&
        alloc_buffers.same_as(sblock->alloc_buffers)) {
      return sblock;

    } else {
      auto n = CopyOnWrite(sblock.get());
...
private:
  ...
  UnionAccessRegion(const ffi::Array<BufferRegion> &regions) const {
    // For now we only allow Buffer access the same elements.
    // e.g. `[A[vi, vj], A[vi, vj]]` is a legal pattern but need to union to
    // `A[vi, vj]` However, `A[vi, vj], A[vi, vj + 1]` is not allow for now.
...
/*! \brief A mutator which detect sblock name duplication and deduplicate the
 * names. */
class BlockNameDeduplicator : public tirx::StmtMutator {
private:
  Stmt VisitStmt_(const SBlockNode *op) final {
    SBlock sblock = Downcast<SBlock>(tirx::StmtMutator::VisitStmt_(op));

    ffi::String name = GetUniqueName(sblock->name_hint);

    if (name == sblock->name_hint) {
      return sblock;

    } else {
      ffi::ObjectPtr<SBlockNode> n = CopyOnWrite(sblock.get());
  ...
  ffi::String GetUniqueName(const ffi::String &prefix) {
    ...
    while (true) {
      ...
      ++counter;
      ICHECK_GT(counter, 0)
          << "Counter overflow when generating unique sblock name for prefix: "
          << prefix;
    }
  }

  /*! \brief The count map to make sblock name unique. */
...
public:
  ...
  static ffi::Map<Expr, tirx::Buffer> Collect(const IRModule &mod,
                                              const Function &func) {
...
private:
  ...
  void CollectVarMapping(const CallNode *call, const Expr &lhs_var,
    ...
    if (lhs_var->IsInstance<TupleNode>()) {
      ...
      CHECK(lhs_var->IsInstance<VarNode>())
    ...
    auto ValidateBufferCompatibility = [this](tirx::Buffer new_buf, Expr expr) {
      if (auto it = relax_to_tir_var_map_.find(expr);
          it != relax_to_tir_var_map_.end()) {
        ...
        if (compatible) {
          for (size_t j = 0; j < old_buf->shape.size(); ++j) {
            ...
            if (a && b) {
              ...
              compatible =
                  compatible &&
                  ffi::StructuralEqual()(old_buf->shape[j], new_buf->shape[j]);
            }
          }
        }
        ICHECK(compatible) << "Incompatible buffers " << old_buf << " and "
                           << new_buf
...
private:
  ...
  void VisitExpr_(const FunctionNode *func) final {
    ...
    for (const auto &param : prim_func_params) {
      if (auto opt = param.as<tirx::Buffer>()) {
        ...
        // readability.
        tirx::Var param =
            tirx::Var("p_" + buffer->name, PrimType(DataType::Handle()));
    ...
    std::unordered_map<tirx::Buffer, size_t, ffi::ObjectPtrHash,
                       ffi::ObjectPtrEqual>
        buffer_to_idx;
    ...
    // numbered separately because the number of output *vars* might differ from
    // the number of outputs if there are in-place inputs
    int out_idx = 0;
    for (size_t i = 0; i < buffers.size(); ++i) {
      ...
      if (auto it = buffer_to_idx.find(buffers[i]); it != buffer_to_idx.end()) {
        ...
        CHECK(!inplace_indices_.count(idx))
            << "In-place index " << idx
            << " used twice! An argument must be aliased.";
        inplace_indices_.insert(idx);
        continue;
      }

      tirx::Var param = tirx::Var("p_output" + std::to_string(out_idx),
  ...
  void VisitExpr_(const CallNode *call) final {
    ...
    // Step 3. Check functions are all schedulable funcs. i.e. the body of func
    // is root sblock
    ...
    const ffi::Array<ffi::Array<PrimExpr>> &output_buffer_shapes =
    ...
    if (call->args.size() > 2) {
      ...
      if (const auto *shape_expr = tir_vars.as<ShapeExprNode>()) {
        ...
        for (size_t i = 0; i < args.size(); ++i) {
          const tirx::Var &param =
              prim_func->params[num_params - args.size() + i];
  ...
  void VisitExpr_(const TupleGetItemNode *tuple_get_item) final {
    ...
    if (it != func_info_.expr2buffers.end()) {
      ...
      end_buf_idx =
          begin_buf_idx +
          GetTotalTensorSize(tuple_sinfo->fields[tuple_get_item->index]);
      func_info_.expr2buffers.Set(ffi::GetRef<Expr>(tuple_get_item),
                                  {(*it).second.begin() + begin_buf_idx,
                                   (*it).second.begin() + end_buf_idx});
    }
  }
   ...
   */
  static ffi::Array<ffi::Array<PrimExpr>>
  GetCallTIROutputShapes(const CallNode *call) {
    static const Op &call_tir_op_ = Op::Get("relax.call_tir");
    static const Op &call_tir_inplace_op_ = Op::Get("relax.call_tir_inplace");
   ...
   * `buffer_subst_map` */
  void MapArgsToBuffer(const ffi::Array<Expr> args,
                       const ffi::Array<tirx::Buffer> &buffers) {
    size_t buffer_idx = 0;
  ...
  static ffi::Array<tirx::Var>
  GetPrimFuncOutputParams(const tirx::PrimFunc &func,
    ...
    for (auto idx : output_indices) {
      ...
      if (param->dtype.is_int() || param->dtype.is_uint()) {
        if (symbolic_var_index == -1)
          symbolic_var_index = i;
      } else if (param->dtype.is_handle()) {
        CHECK(symbolic_var_index == -1)
   ...
   * PrimFunc output(s) are intermediate results.
   * \param expr The relax Expr, which can be binding vars or binding values.
   * \param func The old TIR PrimFunc
   * \param output_shapes The shape of output params.
   */
  void AllocateIntermediateBuffer(
      const CallNode *call, const tirx::PrimFunc &func,
    ...
    ffi::Array<tirx::Var> output_params =
        GetPrimFuncOutputParams(func, output_idxs);
    ...
    for (size_t i = 0; i < output_size; ++i) {
      ...
      if (output_idxs[i].IntValue() < num_inputs) {
        CHECK(input_buffers.has_value())
            << "Inplace functions must have some defined input";
        output_buffers.push_back(
            input_buffers.value()[output_idxs[i].IntValue()]);
        continue;
      }
      ...
      // Keep static buffer dimensions consistent at the Relax-to-TIR
      // boundary. Symbolic dimensions retain their original dtype.
      auto n = ffi::make_object<tirx::BufferNode>(*buffer.get());
      n->shape = NormalizeStaticBufferShape(output_shapes[i]);
   ...
   */
  static void
  CollectPrimFuncParams(const Var &relax_param,
    ...
    if (const auto *tensor = struct_info.as<TensorStructInfoNode>()) {
      ...
      if (tir_buffer_param.defined()) {
        buffer = tirx::decl_buffer(buffer_shape, dtype, name_hint,
      ...
      ICHECK(prim_value->value->IsInstance<tirx::VarNode>());
      ...
      for (const auto &var : shape_expr->values.value()) {
        ICHECK(var->IsInstance<tirx::VarNode>());
  ...
  tirx::PrimFunc ConstructFunc() {
    ...
    tirx::Stmt body =
        tl::BlockNameDeduplicator()(tirx::SeqStmt::Flatten(func_info_.bodies));

    body = subst.Substitute(body);
    body = tirx::SBlock({}, {}, {}, "root", std::move(body), std::nullopt,
                        alloc_buffers);
    body = tirx::SBlockRealize({}, Bool(true), Downcast<tirx::SBlock>(body));
    tirx::PrimFunc func(func_info_.params, body, VoidType(),
                        func_info_.buffer_map, DictAttrs(attr_map));
    // Renew function defs to prevent using the same symbolic vars in different
    // functions
...
std::vector<size_t> GetTupleAccessedIndices(const FunctionNode *func,
                                            const Var &tuple_var) {
  // Need to be ordered
  std::vector<size_t> indices;
  PostOrderVisit(func->body, [&indices, tuple_var](Expr e) {
    if (auto tup_get = e.as<TupleGetItemNode>();
        tup_get && tup_get->tuple.same_as(tuple_var)) {
...
private:
  ...
  static Expr GetCallTIRShape(StructInfo sinfo) {
    if (auto *tuple = sinfo.as<TupleStructInfoNode>()) {
      ffi::Array<Expr> fields =
          tuple->fields.Map([&](StructInfo x) { return GetCallTIRShape(x); });
      return Tuple(fields);
    } else {
  ...
  Expr VisitExpr_(const CallNode *op) final {
    ...
    if (!opt_gvar) {
      // Case 1. The Call isn't a relax-to-relax function call, no need to
      // update.
      return call;
    }
    ...
    for (size_t i = 0; i < call->args.size(); ++i) {
      ...
      ICHECK(!relax_func->params[i]
                  ->struct_info_->IsInstance<TupleStructInfoNode>() &&
          ...
          << ", which expects parameter " << relax_func->params[i]
          << " to have struct info " << relax_func->params[i]->struct_info_;
