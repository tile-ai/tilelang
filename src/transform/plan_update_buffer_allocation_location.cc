...
namespace tvm { namespace tir { ffi::Map<Var, ffi::Optional<Stmt>> DetectBufferVarAccessLCA(const PrimFunc& func); } }
...
class BufferAllocationLocator : public StmtExprMutator {
 public:
  explicit BufferAllocationLocator(const PrimFunc& func) {
    ...
    ffi::Map<Buffer, ffi::Optional<Stmt>> buffer_lca = tir::DetectBufferAccessLCA(func);
    ffi::Map<Var, ffi::Optional<Stmt>> var_lca = tir::DetectBufferVarAccessLCA(func);
    ...
    for (const auto& buffer : buffer_alloc_recorder) {
      // Prefer the LCA derived from the underlying data var. If missing, fall back to Buffer LCA.
 ...
 private:
  ...
  void PopBinding(const Var& v) {
    auto it = buffer_data_to_buffers_.find(v);
    if (it == buffer_data_to_buffers_.end()) return;
    ffi::Array<Buffer> arr = (*it).second;
    if (arr.size() > 0) {
      // erase last element
      std::vector<Buffer> tmp;
      tmp.reserve(arr.size() - 1);
      for (size_t i = 0; i + 1 < arr.size(); ++i) tmp.push_back(arr[i]);
  ...
  Stmt InjectOpaqueBlock(Stmt body, const ffi::Array<Buffer>& alloc_buffers) {
    ...
    ffi::Array<ffi::Array<BufferRegion>> access = GetBlockReadWriteRegion(opaque_block, var_map);
    n->reads = access[0];
    n->writes = access[1];
