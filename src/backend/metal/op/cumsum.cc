/*!
 * \file tl/backend/metal/op/cumsum.cc
 * \brief Metal implementation for tl.cumsum lowering.
 */
...
bool MatchMetalCumSumTarget(Target target) { return TargetIsMetal(target); }

bool RegisterMetalCumSum() {
  RegisterCumSumImpl(CumSumImpl{
      "metal.CumSum",
      MatchMetalCumSumTarget,
      metal::CumSum::Lower,
  });
  return true;
}

const bool metal_cumsum_registered = RegisterMetalCumSum();
