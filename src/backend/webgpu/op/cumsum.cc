/*!
 * \file tl/backend/webgpu/op/cumsum.cc
 * \brief WebGPU implementation for tl.cumsum lowering.
 */
...
bool MatchWebGPUCumSumTarget(Target target) {
  return target.defined() && target->kind.defined() &&
         target->kind->name == "webgpu";
}

bool RegisterWebGPUCumSum() {
  RegisterCumSumImpl(CumSumImpl{
      "webgpu.CumSum",
      MatchWebGPUCumSumTarget,
      webgpu::CumSum::Lower,
  });
  return true;
}

const bool webgpu_cumsum_registered = RegisterWebGPUCumSum();
