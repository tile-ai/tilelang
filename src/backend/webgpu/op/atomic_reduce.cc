/*!
 * \file tl/backend/webgpu/op/atomic_reduce.cc
 * \brief WebGPU implementation for tl.atomicmax/tl.atomicmin lowering.
 */
...
bool MatchWebGPUAtomicReduceTarget(Target target) {
  return target.defined() && target->kind.defined() &&
         target->kind->name == "webgpu";
}

bool RegisterWebGPUAtomicReduce() {
  RegisterAtomicReduceImpl(AtomicReduceImpl{
      "webgpu.AtomicReduce",
      MatchWebGPUAtomicReduceTarget,
      webgpu::AtomicReduce::Lower,
  });
  return true;
}

const bool webgpu_atomic_reduce_registered = RegisterWebGPUAtomicReduce();
