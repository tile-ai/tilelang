...
inline bool TryRemoveThreadOwnedFactor(PrimExpr *expr, IterVar *iter_var,
                                       int64_t factor,
                                       arith::Analyzer *analyzer) {
  ...
  *expr = analyzer->Simplify(
      Substitute(simplified_expr, {{old_var, new_var * factor_expr}}));
  *iter_var = IterVar(Range(0, new_extent), new_var, IterVarType::kDataPar);
...
BuildLocalReduceIterator(const PrimExpr &index_expr,
  ...
  while (remaining_thread_owned_factor > 1) {
    ...
    for (int64_t factor : candidates) {
      if (TryRemoveThreadOwnedFactor(&cur_expr, &cur_iter_var, factor,
                                     &proof_analyzer)) {
