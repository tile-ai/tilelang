/*!
 * \file tl/backend/cpu/op/cumsum.cc
 * \brief CPU implementation for tl.cumsum lowering.
 */
...
bool MatchCPUCumSumTarget(Target target) { return TargetIsCPU(target); }

bool RegisterCPUCumSum() {
  RegisterCumSumImpl(CumSumImpl{
      "cpu.CumSum",
      MatchCPUCumSumTarget,
      cpu::CumSum::Lower,
  });
  return true;
}

const bool cpu_cumsum_registered = RegisterCPUCumSum();
