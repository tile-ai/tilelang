...
def flashattn(batch, heads, heads_kv, dim, dim_v):
    ...
    def kernel_func(
        block_N, block_H, num_split, num_stages, threads, max_cache_seqlen, max_selected_blocks
    ):
                        ...
                        if k == 0:  # assume block_indices is sorted in reverse order, otherwise, remove this if condition
                            for i, j in T.Parallel(block_H, block_N):
                                acc_s[i, j] = T.if_then_else(
                                    i_s * block_N + j >= cache_seqlens[bid],
...
def sparse_gqa_decode_varlen_indice(
    query, key, value, block_indices, cache_seqlens, max_cache_seqlen, block_size
):
    """
    Args:
    ...
    actual_num_blocks = actual_num_blocks[:, 0
