...
def matmul_non_persistent(
    M, N, K, block_M, block_N, block_K, threads, num_stages, dtype="float16", accum_dtype="float"
):
...
def matmul_persistent(
    ...
    def main_persistent_primitive(
        ...
        with T.Kernel(sm_num, threads=threads) as (block_id):
            ...
            for bx, by in T.Persistent([T.ceildiv(M, block_M), T.ceildiv(N, block_N)], sm_num,
                                       block_id):
                T.clear(C_local)
                for k in T.Pipelined(T.ceildiv(K, block_K), num_stages=num_stages):
                    T.copy(A[bx * block_M, k * block_K], A_shared)
...
def main(M=4096, N=4096, K=4096):
    ...
    non_persistent_kernel = matmul_non_persistent(
        M, N, K, BLOCK_M, BLOCK_N, BLOCK_K, threads, num_stages
    )
