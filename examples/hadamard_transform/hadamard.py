...
def hadamard(b, n, dtype):
    ...
    @T.macro
    def warp_shfl(
        local: T.Tensor((thread_elem,), dtype),
        buf: T.Tensor((thread_elem,), dtype),
        round: int
    ):
...
def ref_program(x: torch.Tensor):
    ...
    return F.linear(x, 
        torch.tensor(scipy.linalg.hadamard(dim, dtype=float), dtype=x.dtype, device=x.device))
