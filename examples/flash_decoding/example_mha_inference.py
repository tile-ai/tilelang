...
@tilelang.jit()
def flashattn(Q, K, V, glse, Output_partial, is_causal: bool = False, block_M: int = 128, block_N: int = 64):
    batch, seqlen_q, heads, dim = T.const("batch seqlen_q heads dim")
    _, seqlen_kv, _, _ = T.const("_ seqlen_kv _ _")
    Q: T.Tensor[[batch, seqlen_q, heads, dim], T.float16]
    K: T.Tensor[[batch, seqlen_kv, heads, dim], T.float16]
    V: T.Tensor[[batch, seqlen_kv, heads, dim], T.float16]
    glse: T.Tensor[[batch, heads, num_split, seqlen_q], T.float16]
    Output_partial: T.Tensor[[batch, seqlen_q, heads, num_split, dim], T.float16]
    Output = T.empty([batch, seqlen_q, heads, dim], T.float16)
...
def main(BATCH=1, H=32, Q_CTX=128, KV_CTX=8192, D_HEAD=128, causal=False):
    ...
    q = torch.randn(BATCH, Q_CTX, H, D_HEAD, dtype=dtype, device=device)
    k = torch.randn(BATCH, KV_CTX, H, D_HEAD, dtype=dtype, device=device)
    v = torch.randn(BATCH, KV_CTX, H, D_HEAD, dtype=dtype, device=device)
    glse = torch.empty(BATCH, H, num_split, Q_CTX, dtype=dtype, device=device)
    Output_partial = torch.empty(BATCH, Q_CTX, H, num_split, D_HEAD, dtype=dtype, device=device)

    ref_fn = partial(ref_program, causal=causal)
    profiler = flashattn.compile(q, k, v, glse, Output_partial, is_causal=causal, block_M=BLOCK_M, block_N=BLOCK_N).get_profiler(
