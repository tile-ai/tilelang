    ...
    def flash_attn(
        ...
        with T.Kernel(batch, heads // valid_block_H, num_split, threads=threads) as (bx, by, bz):
            ...
            for k in T.Pipelined(loop_range, num_stages=num_stages):
                T.copy(
                    K[cur_start_k + k * block_N:cur_start_k + (k + 1) * block_N, cur_kv_head, :],
                    K_shared
                )
                T.clear(acc_s)
                ...
                T.copy(
                    V[cur_start_k + k * block_N:cur_start_k + (k + 1) * block_N, cur_kv_head, :],
                    V_shared
                )
...
def flash_attn_with_attn_pool_decode(
    ...
    S = torch.zeros(
        (batch, q_h, math.ceil(max_seqlen_k / block_size)), dtype=Q.dtype, device=Q.device
    )
