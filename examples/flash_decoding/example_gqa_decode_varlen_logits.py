...
def flashattn(
    ...
    total_seqlen_k, k_heads, _ = T.const("total_seqlen_k k_heads _")
    Q: T.Tensor[[batch, heads, dim], T.float16]
    K: T.Tensor[[total_seqlen_k, k_heads, dim], T.float16]
    V: T.Tensor[[total_seqlen_k, k_heads, dim], T.float16]
    cu_seqlens_k: T.Tensor[[batch + 1], T.int32]
    s_aux: T.Tensor[[heads], T.float32]
    Output = T.empty([batch, heads, dim], T.float16)
    S = T.empty([batch, heads, math.ceil(max_seqlen_kv / block_N)], T.float16)
...
def test_varlen_decode_main(args):
    ...
    # Run tilelang kernel
    O_tl, S_tl = flashattn(q, k_varlen, v_varlen, cu_seqlens_k, sink, max_seqlen_kv=max_k_seqlen, has_sink=args.test_sink)
...
def speed_benchmark_decode_comparison(args):
    ...
    def run_once():
        flashattn(q_decode, k_varlen, v_varlen, cu_seqlens_k, sink, max_seqlen_kv=args.k_seqlen, has_sink=args.test_sink)
