...
def postprocess(
    ...
    def main(
        ...
        with T.Kernel(T.ceildiv(S_kv, block_N), kv_group, B, threads=threads) as (bx, by, bz):
            # T.annotate_layout({dKV: make_dkv_layout(dKV)})
            T.copy(
...
def bwd(
    ...
    if q_start_index_s != 0:
        assert q_start_index_s > kv_stride, "If it is because each cp has too short length, you should fix the logic involving CP0"
    ...
    # print(f'padded_H = {padded_H}, H = {H}')
    if padded_H != H:
        assert kv_group == 1, 'here we solve the H padding automically, other wise you should handle with your mask'
    BI = block_I
    ...
    def main(
        ...
        with T.Kernel(S, B, kv_group, threads=threads) as (bx, by, bz):
            Q_shared = T.alloc_shared([padded_H, D], dtype)
            Q_tail_shared = T.alloc_shared([padded_H, D_tail], dtype)
            KV_shared = T.alloc_shared([BI, D], dtype)
            KV_tail_shared = T.alloc_shared([BI, D_tail], dtype)
            dO_shared = T.alloc_shared([padded_H, D], dtype)
            mask = T.alloc_shared([BI], "bool")

            P_shared_cast = T.alloc_shared([padded_H, BI], dtype)
            dP_shared_cast = T.alloc_shared([padded_H, BI], dtype)
            dQ_shared = T.alloc_shared([padded_H, D], dtype)
            dQ_tail_shared = T.alloc_shared([padded_H, D_tail], dtype)

            acc_p = T.alloc_fragment([padded_H, BI], accum_dtype)
            acc_dp = T.alloc_fragment([padded_H, BI], accum_dtype)
            acc_dq = T.alloc_fragment([padded_H, D], accum_dtype)
            acc_dq_tail = T.alloc_fragment([padded_H, D_tail], accum_dtype)
            acc_dkv = T.alloc_fragment([BI, D], accum_dtype)
            acc_dkv_tail = T.alloc_fragment([BI, D_tail], accum_dtype)
            acc_dkv_shared = T.view(KV_shared, shape=[BI // split_store, D], dtype=accum_dtype)
            acc_dkv_tail_shared = T.view(KV_tail_shared, shape=[BI // split_store, D_tail], dtype=accum_dtype)
            ...
            for i_i in T.Pipelined(NI, num_stages=num_stages):
                ...
                for bi_i in T.Parallel(BI):
                    mask[bi_i] = Indices[by, s_i, bz, i_i * BI + bi_i] <= max_kv_i
                ...
                for bi_i, d_i in T.Parallel(BI, D_tail):
                    KV_tail_shared[bi_i, d_i] = KV[by, Indices[by, s_i, bz, i_i * BI + bi_i], bz, D + d_i]
                ...
                # Adjust dP with softmax gradient formula
                for h_i, bi_i in T.Parallel(padded_H, BI):
                ...
                for s in range(split_store):
                    for bi_i, d_i in T.Parallel(BI, D):
                        if bi_i < BI // split_store:
                            acc_dkv_shared[bi_i, d_i] = acc_dkv[bi_i + s * (BI // split_store), d_i]

                    for bi_i, d_i in T.Parallel(BI, D_tail):
                        if bi_i < BI // split_store:
                            acc_dkv_tail_shared[bi_i, d_i] = acc_dkv_tail[bi_i + s * (BI // split_store), d_i]
                    ...
                    for bi_i, d_i in T.Parallel(BI // split_store, D_tail // 4):
                        T.atomic_addx4(dKV[by, Indices[by, s_i, bz, i_i * BI + bi_i + s * (BI // split_store)], bz, D + d_i * 4], acc_dkv_tail_shared[bi_i, d_i * 4])
...
def sparse_mla_bwd(q, kv, o, do, indices, lse, q_start_index_s, kv_stride, sm_scale=None, is_casual=True, return_kernel=False, delta=None):
    ...
    bwd_kernel = bwd(B, S, S_kv, H, D, D_tail, topk, kv_stride, q_start_index_s, kv_group, sm_scale, is_casual)
...
def ref_sparse_mla_bwd_interface(q, kv, o, do, indices, lse, q_start_index_s, kv_stride=4, sm_scale=None, is_casual=True):
    from examples.deepseek_v32.sparse_mla_fwd_pipelined import ref_sparse_mla_fwd_interface
    q = q.detach().clone()
    kv = kv.detach().clone()
    q.requires_grad = True
    kv.requires_grad = True
    o = ref_sparse_mla_fwd_interface(q, kv, indices, q_start_index_s, kv_stride, sm_scale, is_casual)
    o.backward(do)
...
def test_sparse_mla_bwd(B=1,
    ...
    for b in range(B):
        for t in range(S):
            for h in range(HKV):
                i_i = torch.randperm(max(1, ((t + q_start_index_s) // KV_stride)))[:topk]
