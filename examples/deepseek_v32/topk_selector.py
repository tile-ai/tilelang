...
def tl_topk_impl(input, starts, ends, topk: int = 2048, in_dtype=T.float32, out_dtype=T.int32):
    batch, seq_len = T.const("batch seq_len")
    ...
    index = T.empty([batch, topk], out_dtype)
...
def tl_topk(input, starts, ends, topk):
    indexes = tl_topk_impl(input, starts, ends, topk)
    return indexes
