...
def sparse_mla_fwd(
    ...
    batch, seq_len, heads, _ = T.const("batch seq_len heads _")
    _, seq_len_kv, _, _ = T.const("_ seq_len_kv _ _")
    _, _, _, topk = T.const("_ _ _ topk")
    Q: T.Tensor[[batch, seq_len, heads, dim + tail_dim], T.bfloat16]
    KV: T.Tensor[[batch, seq_len_kv, kv_group, dim + tail_dim], T.bfloat16]
    Indices: T.Tensor[[batch, seq_len, kv_group, topk], T.int32]
    q_start_index_s: T.Tensor[[1], T.int32]
...
def sparse_mla_fwd_interface(
    ...
    q_start_index_s_t = torch.tensor([q_start_index_s], dtype=torch.int32, device="cuda")
    if print_kernel:
        kernel = sparse_mla_fwd.compile(q, kv, indices, q_start_index_s_t, dim, tail_dim, kv_stride, kv_group, sm_scale, is_casual, CP0)
        print(kernel.get_kernel_source())
    if return_kernel:
        # Return a callable that matches the old kernel interface
        def kernel_wrapper(q, kv, indices, q_start_index_s_t):
            return sparse_mla_fwd(q, kv, indices, q_start_index_s_t, dim, tail_dim, kv_stride, kv_group, sm_scale, is_casual, CP0)

        return kernel_wrapper
    out, lse = sparse_mla_fwd(q, kv, indices, q_start_index_s_t, dim, tail_dim, kv_stride, kv_group, sm_scale, is_casual, CP0)
...
def run_regression_perf(B=1, S=4096, SKV=8192, H=128, HKV=1, DQK=576, DV=512, topk=2048, dtype=torch.bfloat16, q_start_s_index=1024):
    ...
    def fn():
        sparse_mla_fwd(q, kv, indices, q_start_s_index_t, dim, tail_dim, KV_stride, kv_group, None, True, CP0)
