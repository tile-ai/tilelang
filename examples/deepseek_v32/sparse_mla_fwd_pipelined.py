...
def test_sparse_attn_mla_fwd(test_correctness=False):
    ...
    tl_out, tl_lse = fn()
    if test_correctness:
        ref_out = ref_sparse_attention_fwd_interface(q, kv, indices, q_start_s_index, KV_stride)
        print(f"tl_out: {tl_out}")
        print(f"ref_out: {ref_out}")
        assert_similar(tl_out, ref_out)
...
if __name__ == "__main__":
    ...
    test_sparse_attn_mla_fwd(args.test_correctness)
