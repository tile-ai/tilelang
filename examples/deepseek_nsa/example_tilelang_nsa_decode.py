...
def native_sparse_attention(
    ...
    batch, _, heads, dim = T.const("batch _ heads dim")
    _, seq_len, head_kv, _ = T.const("_ seq_len head_kv _")
    Q: T.Tensor[[batch, 1, heads, dim], T.float16]
    K: T.Tensor[[batch, seq_len, head_kv, dim], T.float16]
    V: T.Tensor[[batch, seq_len, head_kv, dim], T.float16]
    BlockIndices: T.Tensor[[batch, 1, head_kv, selected_blocks], T.int32]
    Output = T.empty([batch, 1, heads, dim], T.float16)
