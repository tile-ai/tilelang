...
def flashattn_bwd_atomic_add_auto(
    batch, total_q, total_kv, N_CTX, heads, max_seq_len, dim_qk, dim_v, is_causal, block_M, block_N, threads=256, num_stages=2, groups=1
):
    return _flashattn_bwd_atomic_body(
        batch, total_q, total_kv, N_CTX, heads, max_seq_len, dim_qk, dim_v, is_causal, block_M, block_N, threads, num_stages, groups
    )
...
def flashattn_bwd_split_auto(
    batch, total_q, total_kv, N_CTX, heads, max_seq_len, dim_qk, dim_v, is_causal, block_M, block_N, threads=256, num_stages=2, groups=1
):
    return _flashattn_bwd_split_body(
        batch, total_q, total_kv, N_CTX, heads, max_seq_len, dim_qk, dim_v, is_causal, block_M, block_N, threads, num_stages, groups
    )
...
def flashattn_bwd_atomic_add_manual(
    batch, total_q, total_kv, N_CTX, heads, max_seq_len, dim_qk, dim_v, is_causal, block_M, block_N, threads=256, num_stages=2, groups=1
):
    return _flashattn_bwd_atomic_body(
        batch, total_q, total_kv, N_CTX, heads, max_seq_len, dim_qk, dim_v, is_causal, block_M, block_N, threads, num_stages, groups
...
def flashattn_bwd_split_manual(
    batch, total_q, total_kv, N_CTX, heads, max_seq_len, dim_qk, dim_v, is_causal, block_M, block_N, threads=256, num_stages=2, groups=1
):
    return _flashattn_bwd_split_body(
        batch, total_q, total_kv, N_CTX, heads, max_seq_len, dim_qk, dim_v, is_causal, block_M, block_N, threads, num_stages, groups
...
def _make_attention_class(use_split, use_auto_schedule):
    ...
    if use_split and use_auto_schedule:
        bwd_kernel_fn = flashattn_bwd_split_auto
    elif use_split and not use_auto_schedule:
        bwd_kernel_fn = flashattn_bwd_split_manual
    elif not use_split and use_auto_schedule:
        bwd_kernel_fn = flashattn_bwd_atomic_add_auto
    else:
        bwd_kernel_fn = flashattn_bwd_atomic_add_manual
    ...
    class _attention(torch.autograd.Function):
        ...
        def backward(ctx, do):
            ...
            do_c, q, k, v, o = [maybe_contiguous(x) for x in (do_unpad, q, k, v, o)]
            block_M = 128
            block_N = 32
            mod_prep = flashattn_bwd_preprocess(BATCH, H, total_q, N_CTX, ctx.max_seqlen_q, D_HEAD_V)
            delta = mod_prep(o, do_c, cu_seqlens_q)
            ...
            if use_split:
                ...
                kernel(q, k, v, do_c, lse_clone, delta, cu_seqlens_q, cu_seqlens_k, dq, dk, dv)
...
def main(
    ...
    # All tilelang configurations to test: (use_split, use_auto_schedule, label)
    configs = [
        (False, True, "atomic + auto_schedule"),
        (False, False, "atomic + manual"),
        (True, True, "split  + auto_schedule"),
        (True, False, "split  + manual"),
    ]
    ...
    # ------- PyTorch reference -------
    print("=" * 70)
    print("Running PyTorch reference backward ...")
    print("=" * 70)
    ...
    latency_torch = do_bench(run_torch, warmup=500)
    tflops_torch = total_flops / latency_torch * 1e-9
    results.append(("PyTorch reference", latency_torch, tflops_torch, True))
    print(f"  latency: {latency_torch:.2f} ms  |  {tflops_torch:.2f} TFlops")

    # ------- TileLang configurations -------
    for use_split, use_auto_schedule, label in configs:
        print("=" * 70)
        print(f"Compiling: {label} ...")
        print("=" * 70)

        attn_cls = _make_attention_class(use_split, use_auto_schedule)
        attn_fn = attn_cls.apply
        ...
        # Run once to compile & get outputs
        O_tl = attn_fn(Q, K, V, seqlens_q, seqlens_k, cu_seqlens_q, cu_seqlens_k, max_seqlen_q, max_seqlen_k, causal, groups)
        ...
        correct = True
        try:
            torch.testing.assert_close(O_tl, O_ref.half(), rtol=1e-2, atol=1e-2)
            torch.testing.assert_close(dQ_tl, dQ_ref, rtol=1e-2, atol=1e-2)
            torch.testing.assert_close(dK_tl, dK_ref, rtol=1e-2, atol=1e-2)
            torch.testing.assert_close(dV_tl, dV_ref, rtol=1e-2, atol=1e-2)
        ...
        print(f"  [{status}] latency: {latency:.2f} ms  |  {tflops:.2f} TFlops")
    ...
    print()
    print("=" * 70)
    print("                    GQA Backward Benchmark Results")
    print(f"  BATCH={BATCH}, H={H}, N_CTX={N_CTX}, D_QK={D_HEAD_QK}, D_V={D_HEAD_V}")
    print(f"  groups={groups}, causal={causal}")
    print("=" * 70)
    print(f"{'Rank':<6}{'Configuration':<30}{'Latency(ms)':>12}{'TFlops':>10}{'Correct':>10}")
    print("-" * 68)
    for rank, (label, lat, tfl, cor) in enumerate(results, 1):
        status = "PASS" if cor else "FAIL"
        print(f"{rank:<6}{label:<30}{lat:>12.2f}{tfl:>10.2f}{status:>10}")
    print("-" * 68)
