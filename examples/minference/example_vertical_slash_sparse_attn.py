...
def _tl_vs_sparse_flashattn(
    ...
    vertical_size, slash_size = T.const("vertical_size slash_size")
    block_M = 64
    block_N = 64
    ...
    BlockCount: T.Tensor[[batch, heads, seq_blocks], int_dtype]
    BlockOffset: T.Tensor[[batch, heads, seq_blocks, slash_size], int_dtype]
    ColumnCount: T.Tensor[[batch, heads, seq_blocks], int_dtype]
    ColumnIndex: T.Tensor[[batch, heads, seq_blocks, vertical_size], int_dtype]
...
def _triton_mixed_sparse_attention(
    q, k, v, seqlens, block_count, block_offset, column_count, column_index, sm_scale, block_size_M=64, block_size_N=64
):
...
def vertical_slash_sparse_attention(query, key, value, v_idx, s_idx, block_size_M=64, block_size_N=64):
    ...
    def run(is_triton: bool = True):
        if is_triton:
            ...
            out = _tl_vs_sparse_flashattn(
                ...
                dim=head_dim,
                vertical_size=v_idx.shape[2],
                slash_size=s_idx.shape[2],
            )
...
def run_regression_perf(batch=1, heads=1, seq_len=16384, head_dim=64, vertical_size=1000, slash_size=200):
    ...
    def run_kernel_only():
        _tl_vs_sparse_flashattn(
            ...
            seq_len=context_size,
            dim=head_dim,
            vertical_size=vertical_topk.shape[-1],
            slash_size=slash.shape[-1],
        )
