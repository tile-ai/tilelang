...
# @tilelang.jit
@tilelang.jit
def matmul_warp_specialize_copy_0_gemm_1(
    A, B, block_M: int = 128, block_N: int = 128, block_K: int = 64, dtype: T.dtype = T.float16, accum_dtype=T.float32
):
    M, N, K = T.const("M N K")
...
def main(M=1024, N=1024, K=1024):
    ...
    c = matmul_warp_specialize_copy_0_gemm_1(a, b, block_M=block_M, block_N=block_N, block_K=block_K)
    ...
    # cuda_source = matmul_warp_specialize_copy_0_gemm_1.get_kernel_source(a, b)
    # print("Generated CUDA kernel:\n", cuda_source)

    # 5.Profile latency with kernel
    kernel = matmul_warp_specialize_copy_0_gemm_1.compile(a, b, block_M=block_M, block_N=block_N, block_K=block_K)
