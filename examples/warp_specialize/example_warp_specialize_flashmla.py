...
@tilelang.jit
def flashattn(Q, Q_pe, KV, K_pe, glse, Output_partial, block_N, block_H, num_split):
    batch, heads, kv_head_num, seqlen_kv, dim, pe_dim = T.const("batch, heads, kv_head_num, seqlen_kv, dim, pe_dim")
    scale = T.pow(dim + pe_dim, -0.5) * 1.44269504  # log2(e)
    dtype = T.float16
    ...
    with T.Kernel(heads // T.min(block_H, kv_group_num), batch, threads=256) as (hid, bid):
        # smem_sQ
