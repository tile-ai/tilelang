...
def tilelang_recompute_w_u_fwd(
    ...
    A: T.Tensor[[B, S, H, BS], output_dtype]
    W = T.empty([B, S, H, DK], output_dtype)
    U = T.empty([B, S, H, DV], output_dtype)
    QG = T.empty([B, S, H, DK], output_dtype)
    KG = T.empty([B, S, H, DK], output_dtype)
...
def run_test(
    ...
    block_S = chunk_size
    W_tilelang, U_tilelang, _, KG_tilelang = tilelang_recompute_w_u_fwd(
        K,
        V,
        Beta,
        G,
