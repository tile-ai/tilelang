...
def chunk_bwd_dqkwg(
    ...
    h: T.Tensor[[B, BS, H, DK, DV], input_dtype]
    dv: T.Tensor[[B, S, H, DV], input_dtype]
    DO: T.Tensor[[B, S, H, DV], input_dtype]
    Dh: T.Tensor[[B, BS, H, DK, DV], input_dtype]
    dq = T.empty([B, S, H, DK], T.float32)
    dk = T.empty([B, S, H, DK], T.float32)
    dw = T.empty([B, S, H, DK], gate_dtype)
    dg = T.empty([B, S, H, DK], gate_dtype)
...
def run_test(
    ...
    tilelang_time = do_bench(
        chunk_bwd_dqkwg, q, k, v_new, g, h, dv, do, dh, scale=scale, chunk_size=chunk_size, input_dtype=input_dtype, gate_dtype=gate_dtype
    )
