...
def tilelang_chunk_bwd_intra(
    ...
    g: T.Tensor[[B, S, H, DK], gate_dtype]
    beta: T.Tensor[[B, S, H], input_dtype]
    dAqk: T.Tensor[[B, S, H, BT], input_dtype]
    dAkk: T.Tensor[[B, S, H, BT], input_dtype]
    dq_in: T.Tensor[[B, S, H, DK], input_dtype]
    dk_in: T.Tensor[[B, S, H, DK], input_dtype]
    db: T.Tensor[[B, S, H], input_dtype]
    dg_in: T.Tensor[[B, S, H, DK], gate_dtype]
    dq2 = T.empty([B, S, H, DK], output_dtype)
    dk2 = T.empty([B, S, H, DK], output_dtype)
    db2 = T.empty([NK, B, S, H], output_dtype)
    dg2 = T.empty([B, S, H, DK], gate_dtype)
    ...
    with T.Kernel(T.ceildiv(DK, block_DK) * NC, NT, B * H, threads=threads) as (i_kc, i_t, i_bh):
        ...
        T.copy(dq_in[bb, i_ti : i_ti + BC, bh, i_k * block_DK : (i_k + 1) * block_DK], dq_shared)
        for i_bc, i_k2 in T.Parallel(BC, block_DK):
        ...
        if i_i < NC_actual - 1:
            ...
            for i_j in T.Pipelined(i_i + 1, NC_actual, num_stages=num_stages):
                ...
                for i_bc, i_k2 in T.Parallel(BC, block_DK):
                    ...
                    gkn_shared[i_bc, i_k2] = T.if_then_else(i_tj + i_bc < S, T.exp2(g_next_shared[i_bc, i_k2] - gn_last_shared[i_k2]), 0.0)
        ...
        # Load dk and dg
        T.copy(dk_in[bb, i_ti : i_ti + BC, bh, i_k * block_DK : (i_k + 1) * block_DK], dk_shared)
        T.copy(dg_in[bb, i_ti : i_ti + BC, bh, i_k * block_DK : (i_k + 1) * block_DK], dg_shared)
...
def run_test(
    ...
    dq_tilelang, dk_tilelang, db_tilelang, dg_tilelang = tilelang_chunk_bwd_intra(
        q,
        k,
        g,
        beta,
