...
def per_token_cast_to_fp8(M, N, blk_m):
    ...
    @T.prim_func
    def per_token_cast(
        X: T.Tensor((M, N), dtype), X_fp8: T.Tensor((M, N), T.float8_e4m3), X_amax: T.Tensor((M, T.ceildiv(N, group_size)), dtype)
    ):
        with T.Kernel(T.ceildiv(M, blk_m), T.ceildiv(N, group_size), threads=128) as (bx, by):
            ...
            y_q_local_fp8 = T.alloc_fragment((blk_m, group_size), T.float8_e4m3)
