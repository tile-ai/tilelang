...
@tilelang.jit
def per_token_cast_to_fp8(X, blk_m: int = 8):
    dtype = T.float
    group_size = 128
    fp8_min = -448.0
    fp8_max = 448.0
...
def main(M=8192, N=8192, blk_m=8):
    ...
    print(per_token_cast_to_fp8.compile(x, blk_m).get_kernel_source())
    profiler = per_token_cast_to_fp8.compile(x, blk_m).get_profiler(tensor_supply_type=tilelang.TensorSupplyType.Randn)
