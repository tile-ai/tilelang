...
def main(do_bench: bool = True):
    ...
    N, K = args.n, args.k
    A = torch.randn(K, dtype=torch.float16, device="cuda")
    B = torch.randn(N, K, dtype=torch.float16, device="cuda")
    check_correctness_and_bench(naive_gemv.compile(A, B, BLOCK_N=128, BLOCK_K=128), N, K, do_bench=do_bench)
    check_correctness_and_bench(naive_splitk_gemv.compile(A, B, BLOCK_N=32, BLOCK_K=32), N, K, do_bench=do_bench)
    check_correctness_and_bench(splitk_gemv.compile(A, B, BLOCK_N=32, BLOCK_K=32, reduce_threads=32), N, K, do_bench=do_bench)
    check_correctness_and_bench(splitk_gemv_vectorized.compile(A, B, BLOCK_N=2, reduce_threads=32), N, K, do_bench=do_bench)
    check_correctness_and_bench(splitk_gemv_vectorized_tvm.compile(A, B, BLOCK_N=2, reduce_threads=32), N, K, do_bench=do_bench)

    a = torch.randn(N, K, dtype=torch.float16, device="cuda")
    x = torch.randn(K, dtype=torch.float16, device="cuda")
    check_correctness_and_bench(gemv_alloc_reducer.compile(a, x, block_M=128, block_N=128), N, K, do_bench=do_bench)

    print("Test passed!")

    if do_bench:
        best_result = get_autotuned_kernel(A, B)
        best_config = best_result.config
        kernel = splitk_gemv_vectorized_tvm.compile(A, B, **best_config)
        profiler = kernel.get_profiler()
        ...
        kernel = gemv_alloc_reducer.compile(a, x)
        profiler = kernel.get_profiler()
...
def run_regression_perf():
    ...
    latency = 0.0
    kernel_list = [
        naive_gemv.compile(A, B, BLOCK_N=128, BLOCK_K=128),
        naive_splitk_gemv.compile(A, B, BLOCK_N=32, BLOCK_K=32),
        splitk_gemv.compile(A, B, BLOCK_N=32, BLOCK_K=32, reduce_threads=32),
        splitk_gemv_vectorized.compile(A, B, BLOCK_N=2, reduce_threads=32),
        splitk_gemv_vectorized_tvm.compile(A, B, BLOCK_N=2, reduce_threads=32),
        gemv_alloc_reducer.compile(a, x, block_M=128, block_N=128),
    ]
