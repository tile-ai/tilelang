...
def sage3_packed_fp4_attention_raw_kernel(
    ...
    *,
    ...
    - ``Out``: the raw-core padded output tensor returned by
      ``fp4attn_cuda.fwd``.
    - ``LSE``: an ABI-reserved output matching ``fp4attn_cuda.fwd``. Neither
      raw core currently materializes it, so its contents are undefined.
