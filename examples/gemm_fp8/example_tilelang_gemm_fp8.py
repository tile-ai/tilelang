...
@tilelang.jit
def matmul(A, B, block_M: int = 128, block_N: int = 128, block_K: int = 64, accum_dtype: T.dtype = T.float32):
    M, N, K = T.const("M N K")
    dtype = A.dtype
...
def run_regression_perf():
    ...
    a = torch.randn(M, K, dtype=torch.float16, device="cuda").to(dtype=torch_dtype)
    b = torch.randn(N, K, dtype=torch.float16, device="cuda").to(dtype=torch_dtype)
    kernel_e4m3 = matmul.compile(a, b)
    ...
    if torch.version.hip is None:
        ...
        a = torch.randn(M, K, dtype=torch.float16, device="cuda").to(dtype=torch_dtype)
        b = torch.randn(N, K, dtype=torch.float16, device="cuda").to(dtype=torch_dtype)
        kernel_e5m2 = matmul.compile(a, b)
