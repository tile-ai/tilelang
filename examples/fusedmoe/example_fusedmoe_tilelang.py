...
def moe_forward_tilelang_shared(
    ...
    def kernel_shared(
        ...
        with T.Kernel(T.ceildiv(num_tokens, block_token), T.ceildiv(dexpert, block_dexpert),
            ...
            for i, j in T.Parallel(block_token, block_dexpert):
                gate_logits_local[i, j] = gate_logits_local[
                    i, j] * (1.0 / (1.0 + T.exp2(-gate_logits_local[i, j] * scale)))
        ...
        with T.Kernel(T.ceildiv(num_tokens, block_token), T.ceildiv(dhidden, block_dhidden),
                      threads=threads) as (bx, by):
...
def moe_forward_tilelang_routed(
    ...
    def kernel(
        ...
        with T.Kernel(M, T.ceildiv(dexpert, block_dexpert), threads=threads) as (bx, by):
            ...
            m_start = m_start_padded - group_padded_offsets[cur_group_idx[0]
                                                           ] + group_offsets[cur_group_idx[0]]
            actual_rows = T.max(
                0,
                T.min(
                    block_token,
                    cur_group_size[0] - (m_start_padded - group_padded_offsets[cur_group_idx[0]])
                )
            )
...
class MoE(nn.Module):
    ...
    def __init__(
        ...
        self.stacked_expert_w_gate = torch.stack(
            [expert.W_gate_weight for expert in self.experts], dim=0
        )
        self.stacked_expert_w_up = torch.stack(
            [expert.W_up_weight for expert in self.experts], dim=0
        )
        self.stacked_expert_w_down = torch.stack(
            [expert.W_down_weight for expert in self.experts], dim=0
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(1, len(group_sizes)):
            group_padded_offsets[i] = group_padded_offsets[i - 1] + math.ceil(
                (counts[i - 1] + 1) / self.padding_M
            ) * self.padding_M

        block_token = 128
        M = math.ceil(
            self.config["batch_size"] * self.config["seq_len"] *
            self.config["n_experts_per_token"] / block_token
        ...
        with torch.cuda.stream(routed_stream):
            ...
            self.routed_kernel(
                self.stacked_expert_tokens, self.stacked_expert_w_gate, self.stacked_expert_w_up,
                self.stacked_expert_w_down, self.stacked_expert_weights, group_sizes, group_offset,
        ...
        with torch.cuda.stream(shared_stream):
            ...
            self.shared_kernel(
                x_flat, self.shared_expert.W_gate_weight, self.shared_expert.W_up_weight,
                self.shared_expert.W_down_weight, self.up_logits_shared, self.expert_output_shared
            )
