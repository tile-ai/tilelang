...
def flashattn(
    ...
    KV: T.Tensor[[batch, seqlen_kv, kv_head_num, dim], T.float8_e4m3fn]
...
if __name__ == "__main__":
    ...
    Q = torch.randn(batch, heads, dim, dtype=dtype_q, device="cuda")
    Q_pe = torch.randn(batch, heads, pe_dim, dtype=dtype_q, device="cuda")
    KV = torch.randn(batch, kv_ctx, kv_heads, dim, dtype=dtype_q, device="cuda").to(dtype_kv)
    K_pe = torch.randn(batch, kv_ctx, kv_heads, pe_dim, dtype=dtype_q, device="cuda")
