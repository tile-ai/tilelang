"""Benchmark script for the SM90 bf16×fp8→bf16 grouped GEMM.

Exercises `example_grouped_gemm_fwd_sm90_fp8_to_bf16.grouped_gemm_sm90_fp8_to_bf16`
...
python benchmark_grouped_gemm_sm90_fp8_to_bf16.py --G 8 --M 1024 --N 4096 --K 4096

# Heterogeneous group sizes
python benchmark_grouped_gemm_sm90_fp8_to_bf16.py --batch_sizes 63,77,111,280 --N 2048 --K 2048

# Sweep multiple shapes (G,M,N,K), one per ';'
python benchmark_grouped_gemm_sm90_fp8_to_bf16.py --shapes "4,512,4096,4096;8,1024,4096,4096"
"""
...
import tilelang
from example_grouped_gemm_fwd_sm90_fp8_to_bf16 import (
    grouped_gemm_sm90_fp8_to_bf16,
    construct_inputs_bf16_fp8,
    torch_gmm_bf16_fp8_ref,
)


# Defaults align with the kernel's defaults.
DEFAULTS = {
    "block_M": 128,
    "block_N": 128,
    "block_K": 64,
...
def main():
    ...
    p.add_argument("--no_check", action="store_true",
                   help="skip correctness check vs torch bf16 reference")
    args = p.parse_args()
    ...
    print(f"Running {len(configs)} shape(s) on {torch.cuda.get_device_name()} "
          f"[SM90 bf16 × fp8→bf16, "
          f"block_M={args.block_M} block_N={args.block_N} block_K={args.block_K} "
          f"stages={args.num_stages} threads={args.threads}]")
