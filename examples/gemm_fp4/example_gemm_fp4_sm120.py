...
def unpack_fp4_storage_to_float(packed: torch.Tensor, rows: int, cols: int) -> torch.Tensor:
    # Host tensors use packed bytes; kernel tensors use T.float4_e2m1fn.
...
def require_sm120():
    ...
    if major < 12:
        raise RuntimeError("SM120 FP4 GEMM requires an SM120+ CUDA device")
...
def matmul_fp4(
    ...
    if K % 32 != 0 or block_K % 32 != 0 or block_K > K:
        raise ValueError("matmul_fp4 requires K and block_K to be multiples of 32 and block_K <= K")
    ...
    @T.prim_func
    def main(
        A: T.Tensor(A_shape, T.float4_e2m1fn),
...
def main():
    ...
    func = matmul_fp4(M, N, K, block_M, block_N, block_K, T.float32, T.float32)
    ...
    torch.manual_seed(0)
    a = torch.randint(0, 256, (M, K // 2), device="cuda", dtype=torch.uint8).view(torch.int8)
    b = torch.randint(0, 256, (N, K // 2), device="cuda", dtype=torch.uint8).view(torch.int8)

    zero = torch.zeros_like(a)
    c_zero = kernel(zero, torch.zeros_like(b))
    assert c_zero.abs().max().item() == 0.0

    c = kernel(a, b)
    ref = unpack_fp4_storage_to_float(a, M, K) @ unpack_fp4_storage_to_float(b, N, K).T
