...
def tilelang_chunk_kda_fwd_intra_token_parallel(
    ...
    def kernel(
        ...
        with T.Kernel(B * S, T.ceildiv(H, block_H), threads=threads) as (bbs, bh): # block_index_bs, block_index_dh
            ...
            Q_i_fragment = T.alloc_fragment((block_H, DK), dtype=input_dtype,)
            K_i_fragment = T.alloc_fragment((block_H, DK), dtype=input_dtype,)
            K_j_fragment = T.alloc_fragment((block_H, DK), dtype=accum_dtype,) 

            Sum_Aqk_fragment = T.alloc_fragment((block_H, ), dtype=accum_dtype,) 
            Sum_Akk_fragment = T.alloc_fragment((block_H, ), dtype=accum_dtype,) 
...
def run_test(
    ...
    tilelang_time = do_bench(kernel, q, k, gk, beta,)
