        ...
        with T.Kernel(T.ceildiv(DK, block_DK) * NC, NT, B * H, threads=threads) as (i_kc, i_t, i_bh):
            ...
            for j in T.Pipelined(loop_length, num_stages=num_stages):
                ...
                for i_bc, i_k2 in T.Parallel(BC, block_DK):
                    ...
                    dq2_fragment[i_bc, i_k2] += T.if_then_else(i_bc >= j, dAqk_col[i_bc] * kgj_fragment[i_bc, i_k2], 0.)
                    dk2_fragment[i_bc, i_k2] += T.if_then_else(i_bc >= j, dAkk_col[i_bc] * kgj_fragment[i_bc, i_k2], 0.)
            ...
            if i_i < NC_actual - 1:
                ...
                for i_j in T.Pipelined(i_i+1, NC_actual, num_stages=num_stages):
                    ...
                    for i_bc, i_k2 in T.Parallel(BC, block_DK):
                        ...
                        gkn_shared[i_bc, i_k2] = T.if_then_else(i_tj + i_bc < S, T.exp2(g_next_shared[i_bc, i_k2] - gn_last_shared[i_k2]),0.)
            ...
            for i_bc, i_k2 in T.Parallel(BC, block_DK):
                dg2_fragment[i_bc, i_k2] = dg2_fragment[i_bc, i_k2] + (dk2_fragment[i_bc, i_k2] - dkt_fragment[i_bc, i_k2]) * k_shared[i_bc, i_k2] + dg_shared[i_bc, i_k2]
...
def run_test(
    ...
    fla_time = do_bench(chunk_kda_bwd_intra,
        q=q,
        k=k,
        g=g,
        beta=beta,
