    ...
    def kernel(
        ...
        with T.Kernel(T.ceildiv(S, block_S), B*H, threads=threads) as (bs, bbh):
            ...
            for i_s1, i_s2 in T.Parallel(block_S, block_S):
                dA_fragment[i_s1, i_s2] = T.if_then_else(i_s1>=i_s2, dA_fragment[i_s1, i_s2] * scale, 0.) # 下三角矩阵
...
def run_test(
    ...
    DO, V_new = prepare_input(
        B, S, H, DV, chunk_size, getattr(torch, input_dtype), getattr(torch, do_dtype)
    )
