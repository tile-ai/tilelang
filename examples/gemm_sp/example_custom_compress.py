...
def compress_kernel(A, A_sp, E, block_M: int = 32, block_K: int = 32, dtype: T.dtype = T.float16, use_cutlass_layout: bool = False):
...
def main(M=1024, N=1024, K=1024, use_cutlass_layout=False, use_torch_compressor=False, accum_dtype=T.float, cfg="4090"):
    ...
    if use_torch_compressor:
        ...
        a_sparse = torch.zeros(M, K // 2, device="cuda", dtype=torch.half)
        e = torch.zeros(M, K // 16, device="cuda", dtype=torch.int16)
        compress_kernel(a, a_sparse, e, block_M=32, block_K=32, dtype=T.float16, use_cutlass_layout=use_cutlass_layout)

    c = matmul_sp_fp16_custom_compress(
        a_sparse, e, b, accum_dtype=accum_dtype, **DEFAULT_CONFIG[cfg][accum_dtype], use_cutlass_layout=use_cutlass_layout
    )
