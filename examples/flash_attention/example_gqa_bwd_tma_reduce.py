...
def flashattn_bwd_postprocess(batch, heads, head_kv, seq_len, dim_qk, dim_v):
    ...
    def flash_bwd_post(
        ...
        with T.Kernel(T.ceildiv(seq_len, blk), head_kv, batch, threads=128) as (bx, by, bz):
            ...
            T.copy(
                dK[bz, bx * blk : (bx + 1) * blk, by, :],
                dK_out[bz, bx * blk : (bx + 1) * blk, by, :],
            )
            T.copy(
                dV[bz, bx * blk : (bx + 1) * blk, by, :],
                dV_out[bz, bx * blk : (bx + 1) * blk, by, :],
            )
    ...
    def flash_bwd(
        ...
        with T.Kernel(heads, T.ceildiv(seq_len, block_M), batch, threads=threads) as (bx, by, bz):
            ...
            for k in T.Pipelined(loop_st, loop_ed, num_stages=num_stages):
                ...
                T.copy(dq, dq_shared)
                T.atomic_add(
                    dQ[bz, k * block_N : (k + 1) * block_N, bx, :], dq_shared, use_tma=True
                )
            T.copy(dv, dv_shared)
            T.atomic_add(
...
def flashattn_bwd_split(
    ...
    def flash_bwd(
        ...
        with T.Kernel(heads, T.ceildiv(seq_len, block_M), batch, threads=threads) as (bx, by, bz):
            ...
            T.copy(dv, dv_shared)
            T.copy(
                dv_shared, dV[bx % groups, bz, by * block_M : (by + 1) * block_M, bx // groups, :]
            )
            T.copy(dk, dk_shared)
...
def main(
    ...
    if causal:
        total_flops *= 0.5
    Q = (
        torch.empty(BATCH, N_CTX, H, D_HEAD_QK, dtype=torch.half, device="cuda")
        .normal_()
        .requires_grad_()
    )
    ...
    head_kv = H // groups
    K = (
        torch.empty(BATCH, N_CTX, head_kv, D_HEAD_QK, dtype=torch.half, device="cuda")
        .normal_()
        .requires_grad_()
    )
    V = (
        torch.empty(BATCH, N_CTX, head_kv, D_HEAD_V, dtype=torch.half, device="cuda")
        .normal_()
        .requires_grad_()
    )
    dO = (
        torch.empty(BATCH, N_CTX, H, D_HEAD_V, dtype=torch.half, device="cuda")
        .normal_()
        .requires_grad_()
    )
