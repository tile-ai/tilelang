...
def flashattn_fwd(Q, K, V, is_causal: bool = False, block_M: int = 128, block_N: int = 64, groups: int = 1):
    batch, seq_len, heads, dim_qk = T.const("batch seq_len heads dim_qk")
    _, _, head_kv, _ = T.const("_ _ head_kv _")
    _, _, _, dim_v = T.const("_ _ _ dim_v")
    Q: T.Tensor[[batch, seq_len, heads, dim_qk], T.float16]
    K: T.Tensor[[batch, seq_len, head_kv, dim_qk], T.float16]
    V: T.Tensor[[batch, seq_len, head_kv, dim_v], T.float16]
    Output = T.empty([batch, seq_len, heads, dim_v], T.float16)
...
def flashattn_bwd_preprocess(O, dO):
    batch, seq_len, heads, dim_v = T.const("batch seq_len heads dim_v")
    O: T.Tensor[[batch, seq_len, heads, dim_v], T.float16]
    dO: T.Tensor[[batch, seq_len, heads, dim_v], T.float16]
...
def flashattn_bwd_postprocess(dQ, dK, dV):
    ...
    _, _, head_kv, _ = T.const("_ _ head_kv _")
    _, _, _, dim_v = T.const("_ _ _ dim_v")
    dQ: T.Tensor[[batch, seq_len, heads, dim_qk], T.float32]
    dK: T.Tensor[[batch, seq_len, head_kv, dim_qk], T.float32]
    dV: T.Tensor[[batch, seq_len, head_kv, dim_v], T.float32]
    dQ_out = T.empty([batch, seq_len, heads, dim_qk], T.float16)
    dK_out = T.empty([batch, seq_len, head_kv, dim_qk], T.float16)
    dV_out = T.empty([batch, seq_len, head_kv, dim_v], T.float16)
...
def flashattn_bwd_split_novarlen(
    Q,
    K,
    V,
    dO,
    ...
    Delta: T.Tensor[[batch, heads, seq_len], T.float32]
    dQ: T.Tensor[[batch, seq_len, heads, dim_qk], T.float32]
    dK: T.Tensor[[groups, batch, seq_len, head_kv, dim_qk], T.float16]
    dV: T.Tensor[[groups, batch, seq_len, head_kv, dim_v], T.float16]
...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, causal, groups=1, use_atomic=True):
        ...
        block_M = 128
        block_N = 64
        o, lse = flashattn_fwd(q, k, v, is_causal=causal, block_M=block_M, block_N=block_N, groups=groups)
