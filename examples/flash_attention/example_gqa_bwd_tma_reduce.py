...
def flashattn_fwd(Q, K, V, is_causal, block_M, block_N, groups=1):
    batch, heads, seq_len, dim_qk, dim_v = T.const("batch, heads, seq_len, dim_qk, dim_v")
    ...
    Output = T.empty([batch, seq_len, heads, dim_v], dtype)
    lse = T.empty([batch, heads, seq_len], accum_dtype)
...
def flashattn_bwd_preprocess(O, dO):
    batch, heads, seq_len, dim_v = T.const("batch, heads, seq_len, dim_v")
    dtype = T.float16
    accum_dtype = T.float32
...
def flashattn_bwd_postprocess(dQ, dK, dV):
    batch, heads, head_kv, seq_len, dim_qk, dim_v = T.const("batch, heads, head_kv, seq_len, dim_qk, dim_v")
    dtype = T.float16
    accum_dtype = T.float32
    q_shape = [batch, seq_len, heads, dim_qk]
    k_shape = [batch, seq_len, head_kv, dim_qk]
...
def flashattn_bwd_atomic_add(Q, K, V, dO, lse, Delta, dQ, dK, dV, is_causal, block_M, block_N, threads=256, num_stages=2, groups=1):
    batch, heads, seq_len, dim_qk, dim_v = T.const("batch, heads, seq_len, dim_qk, dim_v")
...
def flashattn_bwd_split_novarlen(Q, K, V, dO, lse, Delta, dQ, dK, dV, is_causal, block_M, block_N, threads=256, num_stages=2, groups=1):
    batch, heads, seq_len, dim_qk, dim_v = T.const("batch, heads, seq_len, dim_qk, dim_v")
    sm_scale = T.pow(dim_qk, -0.5)
    scale = T.pow(dim_qk, -0.5) * 1.44269504  # log2(e)
    head_kv = heads // groups
...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, causal, groups=1, use_atomic=True):
        block_M = 128
        block_N = 64
        o, lse = flashattn_fwd(q, k, v, causal, block_M, block_N, groups)
    ...
    def backward(ctx, do):
        ...
        if ctx.use_atomic:
            ...
            flashattn_bwd_atomic_add(
                q, k, v, do, lse, delta, dq, dk, dv, ctx.causal, block_M, block_N, threads=256, num_stages=2, groups=groups
            )
        else:
            shape_q = [BATCH, N_CTX, H, D_HEAD_QK]
