...
def flashattn(Q, K, V, is_causal, block_M=64, block_N=64, num_stages=1, threads=128):
    batch, heads, seq_q, seq_kv, dim = T.const("batch, heads, seq_q, seq_kv, dim")
...
def main(
    ...
    if not tune:
        ...
        kernel = flashattn.compile(batch=batch, heads=heads, seq_q=seq_q, seq_kv=seq_kv, dim=dim, is_causal=is_causal)
