...
def flashattn(Q, K, V, is_causal: bool = False, block_M: int = 64, block_N: int = 64, num_stages: int = 1, threads: int = 128):
...
def main(
    ...
    q = torch.randn(batch, heads, seq_q, dim, dtype=dtype, device=device)
    k = torch.randn(batch, heads, seq_kv, dim, dtype=dtype, device=device)
    v = torch.randn(batch, heads, seq_kv, dim, dtype=dtype, device=device)

    if not tune:
        profiler = flashattn.compile(q, k, v, is_causal=is_causal, block_M=64, block_N=64, num_stages=1, threads=128).get_profiler()
        ...
        o = flashattn(q, k, v, is_causal=is_causal)
        best_latency = o.latency
        best_config = o.config
        ref_latency = o.ref_latency
...
def run_regression_perf(
    ...
    k = torch.randn(batch, heads, seq_kv, dim, dtype=dtype, device=device)
    v = torch.randn(batch, heads, seq_kv, dim, dtype=dtype, device=device)
    profiler = flashattn.compile(q, k, v, is_causal=is_causal, block_M=128, block_N=128, num_stages=2, threads=256).get_profiler()
