...
def flashattn_fwd(Q, K, V, is_causal: bool = False, block_M: int = 64, block_N: int = 64):
    batch, seq_len, heads, dim = T.const("batch seq_len heads dim")
    Q: T.Tensor[[batch, seq_len, heads, dim], T.float16]
    K: T.Tensor[[batch, seq_len, heads, dim], T.float16]
    V: T.Tensor[[batch, seq_len, heads, dim], T.float16]
    Output = T.empty([batch, seq_len, heads, dim], T.float16)
...
def flashattn_bwd_postprocess(dQ):
    batch, seq_len, heads, dim = T.const("batch seq_len heads dim")
    dQ: T.Tensor[[batch, seq_len, heads, dim], T.float32]
    dQ_out = T.empty([batch, seq_len, heads, dim], T.float16)
...
def flashattn_bwd(Q, K, V, dO, lse, Delta, dQ, dK, dV, is_causal: bool = False, block_M: int = 64, block_N: int = 64):
    batch, seq_len, heads, dim = T.const("batch seq_len heads dim")
    Q: T.Tensor[[batch, seq_len, heads, dim], T.float16]
    K: T.Tensor[[batch, seq_len, heads, dim], T.float16]
    V: T.Tensor[[batch, seq_len, heads, dim], T.float16]
    ...
    dQ: T.Tensor[[batch, seq_len, heads, dim], T.float32]
    dK: T.Tensor[[batch, seq_len, heads, dim], T.float16]
    dV: T.Tensor[[batch, seq_len, heads, dim], T.float16]
...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, causal):
        ...
        o, lse = flashattn_fwd(q, k, v, is_causal=causal, block_M=block_M, block_N=block_N)
    ...
    def backward(ctx, do):
        ...
        flashattn_bwd(q, k, v, do, lse, delta, dq, dk, dv, is_causal=ctx.causal, block_M=block_M, block_N=block_N)
...
def run_regression_perf():
    ...
    profiler = flashattn_bwd.compile(Q, K, V, dO, lse, Delta, dQ, dK, dV, is_causal=causal, block_M=block_M, block_N=block_N).get_profiler()
