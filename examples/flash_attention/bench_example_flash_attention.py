...
def bench_example_mha_bwd_wgmma_pipelined():
    tilelang.tools.bench.process_func(example_mha_bwd_wgmma_pipelined.main,BATCH=1, H=32, N_CTX=256, D_HEAD=64, causal=False)


def bench_example_gqa_fwd_bshd_wgmma_pipelined():
    tilelang.tools.bench.process_func(example_gqa_fwd_bshd_wgmma_pipelined.main,batch=1, heads=16, seq_len=1024, dim=128, is_causal=False, groups=16, tune=False)


def bench_example_gqa_fwd_bshd():
    tilelang.tools.bench.process_func(example_gqa_fwd_bshd.main,batch=1, heads=16, seq_len=1024, dim=128, is_causal=False, groups=16, tune=False)
...
def bench_example_mha_fwd_bshd_wgmma_pipelined():
    tilelang.tools.bench.process_func(example_mha_fwd_bshd_wgmma_pipelined.main,batch=1, heads=32, seq_len=256)
...
def bench_example_mha_fwd_varlen():
    tilelang.tools.bench.process_func(example_mha_fwd_varlen.main,batch=4, heads=16, seq_len=512, dim=64)
