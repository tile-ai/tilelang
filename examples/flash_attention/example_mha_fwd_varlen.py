...
def flashattn(
    ...
    UKV, _, _ = T.const("UKV _ _")
    (batch_size_plus_1,) = T.const("batch_size_plus_1")
    Q_unpad: T.Tensor[[UQ, heads, dim], T.float16]
    K_unpad: T.Tensor[[UKV, heads, dim], T.float16]
    V_unpad: T.Tensor[[UKV, heads, dim], T.float16]
    ...
    with T.Kernel(T.ceildiv(max_seqlen_q, block_M), heads, batch_size, threads=threads) as (bx, by, bz):
        ...
        for k in T.Pipelined(loop_range, num_stages=num_stages):
            ...
            if is_causal:
                ...
                for i, j in T.Parallel(block_M, block_N):
                    acc_s[i, j] = T.if_then_else((bx * block_M + i >= q_current_seqlen or k * block_N + j >= kv_current_seqlen), -1e9, 0)
...
def main(batch: int = 8, heads: int = 64, seq_len: int = 2048, dim: int = 128, causal: bool = False, tune: bool = False):
    ...
    if tune:
        with set_autotune_inputs(q_unpad, k_unpad, v_unpad, cu_seqlens_q, cu_seqlens_k, max_seqlen_q):
            result = flashattn(q_unpad, k_unpad, v_unpad, cu_seqlens_q, cu_seqlens_k, max_seqlen_q, is_causal=causal)
    else:
        out_unpad = flashattn(
            q_unpad,
            k_unpad,
