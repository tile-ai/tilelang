...
def main(batch: int = 8, heads: int = 64, seq_len: int = 2048, dim: int = 128, causal: bool = False, tune: bool = False):
    ...
    if tune:
        with set_autotune_inputs(q_unpad, k_unpad, v_unpad, cu_seqlens_q, cu_seqlens_k):
            kernel = flashattn.compile(batch_size=batch, UQ=UQ, UKV=UKV, heads=heads, dim=dim, max_seqlen_q=max_seqlen_q, is_causal=causal)
    else:
    ...
    # benchmark
    t = do_bench(lambda: kernel(q_unpad, k_unpad, v_unpad, cu_seqlens_q, cu_seqlens_k))
