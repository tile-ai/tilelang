...
def flashattn_bwd_postprocess(dQ):
    batch, heads, seq_len, dim_qk = T.const("batch, heads, seq_len, dim_qk")
    dtype = T.float16
    accum_dtype = T.float32
...
def flashattn_bwd_split(Q, K, V, dO, lse, Delta, dQ, dK, dV, is_causal, block_M, block_N, threads=256, num_stages=2, groups=1):
    batch, heads, seq_len, dim_qk, dim_v = T.const("batch, heads, seq_len, dim_qk, dim_v")
    sm_scale = T.pow(dim_qk, -0.5)
    scale = T.pow(dim_qk, -0.5) * 1.44269504  # log2(e)
    head_kv = heads // groups
...
class _attention(torch.autograd.Function):
    ...
    def backward(ctx, do):
        ...
        if ctx.use_atomic:
            ...
            flashattn_bwd_split(q, k, v, do, lse, delta, dq, dk, dv, ctx.causal, block_M, block_N, threads=256, num_stages=2, groups=groups)
            dq = flashattn_bwd_postprocess(dq)
