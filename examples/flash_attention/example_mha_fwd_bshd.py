...
def flashattn(Q, K, V, is_causal, block_M=64, block_N=64, num_stages=1, threads=128):
...
def main(
    ...
    if not tune:
        kernel = flashattn.compile(
            batch=batch, heads=heads, seq_len=seq_len, dim=dim, is_causal=is_causal, block_M=128, block_N=128, num_stages=1, threads=128
        )
        ...
        best_result = flashattn.compile(batch=batch, heads=heads, seq_len=seq_len, dim=dim, is_causal=is_causal)
