...
def main(
    ...
    k = torch.randn(batch, seq_len, heads, dim, dtype=dtype, device=device)
    v = torch.randn(batch, seq_len, heads, dim, dtype=dtype, device=device)

    if not tune:
        profiler = flashattn.compile(q, k, v, is_causal=is_causal, block_M=128, block_N=128, num_stages=1, threads=128).get_profiler()
        ...
        best_result = flashattn(q, k, v, is_causal=is_causal)
