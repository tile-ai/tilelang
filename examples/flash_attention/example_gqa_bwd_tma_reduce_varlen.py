...
def flashattn_fwd(
    ...
    total_q, heads, dim_qk = T.const("total_q heads dim_qk")
    total_kv, head_kv, _ = T.const("total_kv head_kv _")
    _, _, dim_v = T.const("_ _ dim_v")
    (batch_plus1,) = T.const("batch_plus1")
    Q: T.Tensor[[total_q, heads, dim_qk], T.float16]
    K: T.Tensor[[total_kv, head_kv, dim_qk], T.float16]
    V: T.Tensor[[total_kv, head_kv, dim_v], T.float16]
    cu_seqlens_q: T.Tensor[[batch_plus1], T.int32]
    cu_seqlens_k: T.Tensor[[batch_plus1], T.int32]
    Output = T.empty([total_q, heads, dim_v], T.float16)
    lse = T.empty([batch_plus1 - 1, heads, N_CTX], T.float32)
...
def flashattn_bwd_preprocess(O, dO, cu_seqlens_q, max_seq_len: int = 256, N_CTX: int = 256):
    total_q, heads, dim_v = T.const("total_q heads dim_v")
    (batch_plus1,) = T.const("batch_plus1")
    O: T.Tensor[[total_q, heads, dim_v], T.float16]
    dO: T.Tensor[[total_q, heads, dim_v], T.float16]
    cu_seqlens_q: T.Tensor[[batch_plus1], T.int32]
    Delta = T.empty([batch_plus1 - 1, heads, N_CTX], T.float32)
...
def flashattn_bwd_postprocess(dQ, dK, dV):
    total_q, heads, dim_qk = T.const("total_q heads dim_qk")
    total_kv, head_kv, _ = T.const("total_kv head_kv _")
    _, _, dim_v = T.const("_ _ dim_v")
    dQ: T.Tensor[[total_q, heads, dim_qk], T.float32]
    dK: T.Tensor[[total_kv, head_kv, dim_qk], T.float32]
    dV: T.Tensor[[total_kv, head_kv, dim_v], T.float32]
    dQ_out = T.empty([total_q, heads, dim_qk], T.float16)
    dK_out = T.empty([total_kv, head_kv, dim_qk], T.float16)
    dV_out = T.empty([total_kv, head_kv, dim_v], T.float16)
...
def flashattn_bwd_atomic_add(
    ...
    V: T.Tensor[[total_kv, head_kv, dim_v], T.float16]
    dO: T.Tensor[[total_q, heads, dim_v], T.float16]
    lse: T.Tensor[[batch_plus1 - 1, heads, N_CTX], T.float32]
    Delta: T.Tensor[[batch_plus1 - 1, heads, N_CTX], T.float32]
    cu_seqlens_q: T.Tensor[[batch_plus1], T.int32]
    cu_seqlens_k: T.Tensor[[batch_plus1], T.int32]
    dQ: T.Tensor[[total_q, heads, dim_qk], T.float32]
    dK: T.Tensor[[total_kv, head_kv, dim_qk], T.float32]
...
def flashattn_bwd_split(
    ...
    cu_seqlens_k: T.Tensor[[batch_plus1], T.int32]
    dQ: T.Tensor[[total_q, heads, dim_qk], T.float32]
    dK: T.Tensor[[groups, total_kv, head_kv, dim_qk], T.float16]
    dV: T.Tensor[[groups, total_kv, head_kv, dim_v], T.float16]
...
class _attention(torch.autograd.Function):
    ...
    def backward(ctx, do):
        ...
        block_M = 128
        block_N = 32
        delta = flashattn_bwd_preprocess(o, do, cu_seqlens_q, max_seq_len=ctx.max_seqlen_q, N_CTX=N_CTX)
        ...
        if ctx.use_atomic:
            ...
            dq, _, _ = flashattn_bwd_postprocess(dq, torch.zeros_like(k, dtype=torch.float32), torch.zeros_like(v, dtype=torch.float32))
...
def run_regression_perf():
    ...
    delta = flashattn_bwd_preprocess(O, dO, cu_seqlens_q, max_seq_len=max_seqlen_q, N_CTX=N_CTX)
    dQ = torch.zeros_like(Q, dtype=torch.float32)
    ...
    profiler = flashattn_bwd_split.compile(
        Q,
        K,
        V,
        dO,
