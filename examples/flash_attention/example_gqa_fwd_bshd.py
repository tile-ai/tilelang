...
def flashattn(
    Q, K, V, is_causal: bool = False, groups: int = 1, block_M: int = 64, block_N: int = 64, num_stages: int = 0, threads: int = 128
):
...
def main(
    ...
    k = torch.randn(batch, seq_len, head_kv, dim, dtype=dtype, device=device)
    v = torch.randn(batch, seq_len, head_kv, dim, dtype=dtype, device=device)

    if not tune:
        profiler = flashattn.compile(
            q, k, v, is_causal=is_causal, groups=groups, block_M=64, block_N=64, num_stages=2, threads=128
        ).get_profiler(tensor_supply_type=tilelang.TensorSupplyType.Normal)
