...
def flashattn(Q, K, V, is_causal, groups=1, block_M=64, block_N=64, num_stages=0, threads=128):
...
def main(
    ...
    if not tune:
        ...
        kernel = flashattn.compile(batch=batch, heads=heads, seq_len=seq_len, dim=dim, is_causal=is_causal)
