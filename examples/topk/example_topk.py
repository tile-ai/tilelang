...
def tl_topk(
    ...
    topk_gates = T.empty([M, topk], T.float32)
    topk_indices = T.empty([M, topk], T.int32)

    with T.Kernel(T.ceildiv(M, blk_m), threads=threads) as bx:
        logits_frag = T.alloc_fragment([blk_m, N], dtype=T.float32)
        max_val = T.alloc_fragment([blk_m], dtype=T.float32)
...
def main(argv=None):
    ...
    tl_gates, tl_indices = tl_topk(logits, topk=topk, blk_m=blk_m)
    ...
    # profile
    kernel = tl_topk.compile(logits, topk=topk, blk_m=blk_m)
