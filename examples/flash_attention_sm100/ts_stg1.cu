...
extern "C" __global__ void __launch_bounds__(384, 1) main_kernel(__grid_constant__ const CUtensorMap K_desc, __grid_constant__ const CUtensorMap Output_desc, __grid_constant__ const CUtensorMap Q_desc, __grid_constant__ const CUtensorMap V_desc) {
  ...
  if (tl::tl_shuffle_elect<384>()) {
    mbarrier[4].arrive_and_expect_tx(32768);
    tl::tma_load(Q_desc, mbarrier[4], (&(((bfloat16_t*)buf_dyn_shmem)[0])), 0, ((int)blockIdx.y), (((int)blockIdx.x) * 128), ((int)blockIdx.z));
    tl::tma_load(Q_desc, mbarrier[4], (&(((bfloat16_t*)buf_dyn_shmem)[8192])), 64, ((int)blockIdx.y), (((int)blockIdx.x) * 128), ((int)blockIdx.z));
  }
  if (256 <= ((int)threadIdx.x)) {
    ...
    for (int k = 0; k < 32; ++k) {
      ...
      if (tl::tl_shuffle_elect<128>()) {
        mbarrier[0].arrive_and_expect_tx(32768);
        tl::tma_load(K_desc, mbarrier[0], (&(((bfloat16_t*)buf_dyn_shmem)[16384])), 0, ((int)blockIdx.y), (k * 128), ((int)blockIdx.z));
        tl::tma_load(K_desc, mbarrier[0], (&(((bfloat16_t*)buf_dyn_shmem)[24576])), 64, ((int)blockIdx.y), (k * 128), ((int)blockIdx.z));
      }
      mbarrier[3].wait(((k & 1) ^ 1));
      if (tl::tl_shuffle_elect<128>()) {
        mbarrier[1].arrive_and_expect_tx(32768);
        tl::tma_load(V_desc, mbarrier[1], (&(((bfloat16_t*)buf_dyn_shmem)[32768])), 0, ((int)blockIdx.y), (k * 128), ((int)blockIdx.z));
        tl::tma_load(V_desc, mbarrier[1], (&(((bfloat16_t*)buf_dyn_shmem)[40960])), 64, ((int)blockIdx.y), (k * 128), ((int)blockIdx.z));
      }
    }
  } else {
    ...
    for (int k_1 = 0; k_1 < 32; ++k_1) {
      ...
      tl::__sync_thread_partial<3, 256>();
      tl::tcgen05_after_thread_sync();
      scores_max_clear[0] = tl::AllReduce<tl::MaxOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_max_clear[0], (&(((float*)buf_dyn_shmem)[24832])));
      ...
      scores_sum[0] = tl::AllReduce<tl::SumOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_sum[0], (&(((float*)buf_dyn_shmem)[24576])));
