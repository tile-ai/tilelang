...
def blocksparse_flashattn(Q, K, V, BlockSparseMask, is_causal):
    batch, heads, seq_q, seq_kv, dim, downsample_len = T.const("batch, heads, seq_q, seq_kv, dim, downsample_len")
    block_M = 64
    block_N = 64
    num_stages = 1
    threads = 128
...
def test_topk_sparse_attention():
    ...
    # Run tilelang kernel
    tilelang_output = blocksparse_flashattn(q, k, v, block_mask.to(torch.int8), is_causal=True)
...
def test_topk_sparse_attention_qlen_lt_klen():
    ...
    print(blocksparse_flashattn.get_kernel_source(q, k, v, block_mask.to(torch.int8), is_causal=True))
    tilelang_output = blocksparse_flashattn(q, k, v, block_mask.to(torch.int8), is_causal=True)
...
def run_regression_perf():
    ...
    latency_1 = do_bench(lambda: blocksparse_flashattn(q, k, v, block_mask.to(torch.int8), is_causal=True), backend="cupti")
    ...
    latency_2 = do_bench(lambda: blocksparse_flashattn(q, k, v, block_mask.to(torch.int8), is_causal=True), backend="cupti")
