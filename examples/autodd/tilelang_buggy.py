...
@tilelang.jit
def buggy_matmul(A, B, C, block_M: int = 128, block_N: int = 128, block_K: int = 32, dtype: T.dtype = T.float16, accum_dtype=T.float32):
    M, N, K = T.const("M N K")
...
def run_kernel(config):
    ...
    buggy_matmul(a, b, c, block_M=block_M, block_N=block_N, block_K=block_K)
