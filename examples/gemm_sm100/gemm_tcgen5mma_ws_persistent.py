...
def gemm(
    ...
    assert K % (2 * block_K) == 0  # for simplicty
            ...
            for w in T.unroll(waves):
                ...
                if bx * block_M < M and by * block_N < N:
                    for k in T.serial(k_blocks):
                        ...
                        T.copy(A[bx * block_M: (bx + 1) * block_M, k * block_K: (k + 1) * block_K], A_shared[k % num_stages, :, :])  # cannot use BufferLoad here
            ...
            for w in T.unroll(waves):
                ...
                if bx * block_M < M and by * block_N < N:
                    ...
                    for k in T.serial(k_blocks):
                        ...
                        if w & 1 == 0:
                            T.gemm(A_shared[k % num_stages, :, :], B_shared[k % num_stages, :, :], C_tmem_0, False, False, mbar=consumed[k % num_stages], wg_wait=-1, clear_accum=k == 0)
                        else:
                            T.gemm(A_shared[k % num_stages, :, :], B_shared[k % num_stages, :, :], C_tmem_1, False, False, mbar=consumed[k % num_stages], wg_wait=-1, clear_accum=k == 0)
...
def main():
    ...
    tl_latency = do_bench(lambda: gemm(a, b, block_M, block_N, store_block_N, block_K, in_dtype, out_dtype, accum_dtype, num_stages), backend="cupti")
