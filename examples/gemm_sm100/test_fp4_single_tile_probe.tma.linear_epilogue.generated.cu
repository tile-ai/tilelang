...
extern "C" __global__ void __launch_bounds__(128, 1) main_kernel(__grid_constant__ const CUtensorMap A_desc, __grid_constant__ const CUtensorMap B_desc, float* __restrict__ C) {
  ...
  tl::fence_proxy_async();
  {
    ...
    if ((((int)threadIdx.x) >> 5) == 0) {
      ...
      tl::initialize_tcgen05_descriptor(desc_b, (&(((fp4_e2_t*)buf_dyn_shmem)[16384])), 1, 64, 0, 0, 2);
      #pragma unroll
  ...
  #pragma unroll
  for (int i = 0; i < 8; ++i) {
    tl::store_global_256(&(*(ulonglong4*)(C + ((((int)threadIdx.x) * 64) + (i * 8)))), *(ulonglong4*)(C_local + (i * 8)));
  }
  if ((((int)threadIdx.x) >> 5) == 0) {
