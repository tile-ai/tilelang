...
@tilelang.jit
def flashattn_bwd_dsink(Sinks, Delta, lse, block=128, dtype: T.dtype = T.float16):
    ...
    dsinks = T.empty(shape, accum_dtype)
...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, sinks, window_size):
        ...
        o, lse = flashattn_fwd(q, k, v, sinks, window_size, dtype=dtype)
    ...
    def backward(ctx, do):
        ...
        flashattn_bwd(q, k, v, do, lse, delta, dq, dk, dv, ctx.window_size, dtype=dtype)
...
def run_regression_perf(
    ...
    with torch.no_grad():
        ...
        dO = torch.randn_like(Q)
        O, lse = flashattn_fwd(Q, K, V, sinks, window_size=window_size, dtype=dtype)
        ...
        flashattn_bwd(q, k, v, do, lse, Delta, dq, dk, dv, dtype=dtype)
        _ = flashattn_bwd_dsink(sinks_c, Delta, lse, dtype=dtype).sum(0).sum(1)

        def run_kernel_only():
            flashattn_bwd(q, k, v, do, lse, Delta, dq, dk, dv, window_size, dtype=dtype)
