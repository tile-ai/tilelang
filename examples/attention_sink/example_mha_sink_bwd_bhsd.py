...
@tilelang.jit()
def flashattn_bwd_dsink(Sinks, Delta, lse, block: int = 128):
    heads = T.const("heads")
    batch, _, seq_len = T.const("batch _ seq_len")
    Sinks: T.Tensor[[heads], T.float16]
    Delta: T.Tensor[[batch, heads, seq_len], T.float32]
    lse: T.Tensor[[batch, heads, seq_len], T.float32]
    dsinks = T.empty([batch, heads, seq_len], T.float32)
...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, sinks, window_size):
        ...
        o, lse = flashattn_fwd(q, k, v, sinks, window_size)
    ...
    def backward(ctx, do):
        ...
        flashattn_bwd(q, k, v, do, lse, delta, dq, dk, dv, ctx.window_size)
...
def run_regression_perf(
    ...
    with torch.no_grad():
        ...
        dO = torch.randn_like(Q)
        O, lse = flashattn_fwd(Q, K, V, sinks, window_size=window_size)
        ...
        flashattn_bwd(q, k, v, do, lse, Delta, dq, dk, dv, window_size)
