...
def flashattn_fwd(
    ...
    def flash_fwd(
        ...
        with T.Kernel(T.ceildiv(seq_len, block_M), heads, batch, threads=threads) as (bx, by, bz):
            ...
            start = T.max(
                0, (bx * block_M - window_size) // block_N
                ...
                for i, j in T.Parallel(block_M, block_N):
                    ...
                    if window_size is not None:
                        acc_s[i, j] = T.if_then_else(
                            q_idx >= k_idx and q_idx < k_idx + window_size, 0,
            ...
            for i in T.Parallel(block_M):
                logsum[i] += T.exp2(
                    sinks[i] * 1.44269504 - scores_max[i] * scale
...
def make_dq_layout(dQ):
    ...
    return T.Layout(
        dQ.shape, lambda b, h, l, d: [b, h, l // 8, d // 8, (d % 2), 4 * (l % 8) + (d % 8) // 2]
    )
...
def flashattn_bwd(
    ...
    def flash_bwd(
        ...
        with T.Kernel(heads, T.ceildiv(seq_len, block_M), batch, threads=threads) as (bx, by, bz):
            ...
            loop_ed = T.min(
                T.ceildiv((by + 1) * block_M + window_size, block_N), T.ceildiv(seq_len, block_N)
            ) if window_size is not None else T.ceildiv(seq_len, block_N)
...
def main(
    ...
    if window_size is not None:
        ...
        flops_per_matmul = 2.0 * BATCH * H * min(
            window_size, N_CTX // 2
        ) * N_CTX * D_HEAD  # just a rough estimation
    else:
    ...
    K = torch.randn(
        BATCH, H // groups, N_CTX, D_HEAD, dtype=torch_dtype, device="cuda"
    ).requires_grad_()
    ...
    assert torch.allclose(
        dV, dV_ref, rtol=rtol, atol=atol
    ), f'dV max err: {(dV-dV_ref).abs().max()}'
    assert torch.allclose(
        dK, dK_ref, rtol=rtol, atol=atol
    ), f'dK max err: {(dK-dK_ref).abs().max()}'
    assert torch.allclose(
        dQ, dQ_ref, rtol=rtol, atol=atol
    ), f'dq max err: {(dQ-dQ_ref).abs().max()}'
    assert torch.allclose(
        dsinks, dsinks_ref, rtol=rtol, atol=atol
    ), f'dsinks max err: {(dsinks-dsinks_ref).abs().max()}'
