...
def flashattn_bwd_preprocess(O, dO, dtype: T.dtype = T.float16):
    batch, heads, seq_len, dim = T.const("batch, heads, seq_len, dim")
    accum_dtype = T.float32
...
def flashattn_bwd(Q, K, V, dO, lse, Delta, dQ, dK, dV, groups, window_size=None, sm_scale=None, dtype=T.float16):  # None for full attention
    batch, heads, seq_len, dim = T.const("batch, heads, seq_len, dim")
    head_kv = heads // groups
    q_shape = [batch, heads, seq_len, dim]
    kv_shape = [batch, head_kv, seq_len, dim]
...
@tilelang.jit
def flashattn_bwd_dsink(Sinks, Delta, lse, block=256, dtype: T.dtype = T.float16):
...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, sinks, window_size, groups):
        ...
        o, lse = flashattn_fwd(q, k, v, sinks, groups, window_size, dtype=dtype)
    ...
    def backward(ctx, do):
        ...
        delta = flashattn_bwd_preprocess(o, do, dtype=dtype)
        ...
        flashattn_bwd(q, k, v, do, lse, delta, dq, dk, dv, groups, ctx.window_size, dtype=dtype)
        dq = flashattn_bwd_postprocess(dq, dtype=dtype)

        dsinks = flashattn_bwd_dsink(sinks, delta, lse, dtype=dtype).sum(0).sum(1)
...
def run_regression_perf(
    ...
    with torch.no_grad():
        ...
        flashattn_bwd(q, k, v, do, lse, Delta, dq, dk, dv, groups, window_size, dtype=dtype)
        _ = flashattn_bwd_dsink(sinks_c, Delta, lse, dtype=dtype).sum(0).sum(1)
