...
def flashattn_fwd(
    ...
    UQ, heads, dim = T.const("UQ heads dim")
    UKV, head_kv, _ = T.const("UKV head_kv _")
    batch_size_plus_1 = T.const("batch_size_plus_1")
    Q_unpad: T.Tensor[[UQ, heads, dim], T.float16]
    K_unpad: T.Tensor[[UKV, head_kv, dim], T.float16]
    V_unpad: T.Tensor[[UKV, head_kv, dim], T.float16]
    cu_seqlens_q: T.Tensor[[batch_size_plus_1], T.int32]
    cu_seqlens_k: T.Tensor[[batch_size_plus_1], T.int32]
    Sinks: T.Tensor[[heads], T.float16]
    Output_unpad = T.empty([UQ, heads, dim], T.float16)
    lse = T.empty([batch_size_plus_1 - 1, heads, N_CTX], T.float32)

    batch_size = batch_size_plus_1 - 1
...
def flashattn_bwd_preprocess(O, dO, cu_seqlens_q, max_seq_len: int, N_CTX: int):
    UQ, heads, dim = T.const("UQ heads dim")
    batch_size_plus_1 = T.const("batch_size_plus_1")
    O: T.Tensor[[UQ, heads, dim], T.float16]
    dO: T.Tensor[[UQ, heads, dim], T.float16]
    cu_seqlens_q: T.Tensor[[batch_size_plus_1], T.int32]
    Delta = T.empty([batch_size_plus_1 - 1, heads, N_CTX], T.float32)
...
def flashattn_bwd_postprocess(dQ):
    UQ, heads, dim = T.const("UQ heads dim")
    dQ: T.Tensor[[UQ, heads, dim], T.float32]
    dQ_out = T.empty([UQ, heads, dim], T.float16)
...
def flashattn_bwd(
    ...
    UKV, head_kv, _ = T.const("UKV head_kv _")
    batch_size_plus_1 = T.const("batch_size_plus_1")
    Q: T.Tensor[[UQ, heads, dim], T.float16]
    K: T.Tensor[[UKV, head_kv, dim], T.float16]
    V: T.Tensor[[UKV, head_kv, dim], T.float16]
    dO: T.Tensor[[UQ, heads, dim], T.float16]
    lse: T.Tensor[[batch_size_plus_1 - 1, heads, N_CTX], T.float32]
    Delta: T.Tensor[[batch_size_plus_1 - 1, heads, N_CTX], T.float32]
    cu_seqlens_q: T.Tensor[[batch_size_plus_1], T.int32]
    cu_seqlens_k: T.Tensor[[batch_size_plus_1], T.int32]
    dQ: T.Tensor[[UQ, heads, dim], T.float32]
    dK: T.Tensor[[UKV, head_kv, dim], T.float32]
    dV: T.Tensor[[UKV, head_kv, dim], T.float32]
...
@tilelang.jit()
def flashattn_bwd_dsink(Sinks, Delta, lse, cu_seqlens_q, max_seq_len: int, N_CTX: int, block: int = 256):
    heads = T.const("heads")
    batch_size_plus_1 = T.const("batch_size_plus_1")
    Sinks: T.Tensor[[heads], T.float16]
    Delta: T.Tensor[[batch_size_plus_1 - 1, heads, N_CTX], T.float32]
    lse: T.Tensor[[batch_size_plus_1 - 1, heads, N_CTX], T.float32]
    cu_seqlens_q: T.Tensor[[batch_size_plus_1], T.int32]
    dsinks = T.empty([batch_size_plus_1 - 1, heads, N_CTX], T.float16)
...
class _attention(torch.autograd.Function):
    ...
    def forward(
        ...
        ctx.batch_size = cu_seqlens_q.shape[0] - 1
        return o_unpad
    ...
    def backward(ctx, do):
        ...
        delta = flashattn_bwd_preprocess(o_unpad, do, cu_seqlens_q, ctx.max_seqlen_q, ctx.N_CTX)
        ...
        dsinks = flashattn_bwd_dsink(sinks, delta, lse, cu_seqlens_q, ctx.max_seqlen_q, ctx.N_CTX).sum(0).sum(1)
