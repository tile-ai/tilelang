...
def flashattn(
        ...
        for i, j in T.Parallel(block_M, block_N):
            ...
            if window_size is not None:
                acc_s[i, j] = T.if_then_else(
                    q_idx >= k_idx and q_idx < k_idx + window_size, 0, -T.infinity(acc_s.dtype)
                )
            else:
    ...
    def main(
        ...
        with T.Kernel(T.ceildiv(seq_q, block_M), heads, batch, threads=threads) as (bx, by, bz):
            ...
            start = T.max(
                0, (bx * block_M + past_len - window_size) // block_N
            ) if window_size is not None else 0

            for k in T.Pipelined(start, end, num_stages=num_stages, order=[-1, 0, 3, 1, -1, 2],
...
if __name__ == "__main__":
    ...
    main(
        args.batch, args.heads, args.seq_q, args.seq_kv, args.dim, args.window_size, args.dtype,
        args.tune
    )
