...
def tl_indexer_topk_reducesum_impl(
    ...
    TopkIndices = T.empty(topk_indices_shape, INT32)
    ReduceSum = T.empty(topk_indices_shape, FP32)
    ...
    with T.Kernel(seq_len, threads=num_threads) as (bx):
        ...
        for bk_i in T.Pipelined(num_blocks, num_stages=num_stages):
            ...
            if k_ed > topk and k_ed % topk == 0:
                bitonic_sort(topk_index_shared, topk_value_shared, N, num_iters)
...
def indexer_topk_reducesum_interface(
    ...
    topk_indices, topk_score = tl_indexer_topk_reducesum_impl(q, weights, k, offsets, token_indices, topk=topk, dtype=dtype)
