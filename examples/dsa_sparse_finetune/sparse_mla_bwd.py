...
def preprocess(
    ...
    O: T.Tensor[[S, H, D], dtype]
    dO: T.Tensor[[S, H, D], dtype]
...
def postprocess(
    ...
    dKV: T.Tensor[[S_kv, kv_group, D + D_tail], accum_dtype]
    dKV_out = T.empty([S_kv, kv_group, D + D_tail], dtype)
...
def bwd(
    ...
    Q: T.Tensor[[S, H_orig, D + D_tail], dtype]
    KV: T.Tensor[[S, kv_group, D + D_tail], dtype]
    dO: T.Tensor[[S, H_orig, D], dtype]
    Indices: T.Tensor[[S, kv_group, topk], indices_dtype]
    Lse: T.Tensor[[S, H_orig], accum_dtype]
    Delta: T.Tensor[[S, H_orig], accum_dtype]
    Offsets: T.Tensor[[B_plus_one], indices_dtype]
    TokenIndices: T.Tensor[[S, 2], indices_dtype]
    dQ = T.empty([S, H_orig, D + D_tail], dtype)
    dKV: T.Tensor[[S, kv_group, D + D_tail], accum_dtype]
...
def sparse_mla_bwd(q, kv, o, do, indices, lse, offsets, sm_scale=None, is_casual=True, return_kernel=False, delta=None):
    ...
    dkv = postprocess(dkv, D=D, D_tail=D_tail, kv_group=kv_group)
