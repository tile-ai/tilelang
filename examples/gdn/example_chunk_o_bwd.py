...
def tilelang_chunk_o_bwd_dqkwg(
    ...
    def kernel(
        ...
        with T.Kernel(T.ceildiv(DK, block_DK), T.ceildiv(S, block_S), B * H,
            ...
            T.copy(
                Q[bb, bs * block_S:(bs + 1) * block_S, bh, bk * block_DK:(bk + 1) * block_DK],
                q_shared
            )
            T.copy(
                K[bb, bs * block_S:(bs + 1) * block_S, bh, bk * block_DK:(bk + 1) * block_DK],
                k_shared
            )
            ...
            if use_g:
                ...
                for i_s, i_k in T.Parallel(block_S, block_DK):
                    dq_fragment[
                        i_s,
                        i_k] = dq_fragment[i_s, i_k] * T.exp(G[bb, bs * block_S + i_s, bh]) * scale
                ...
                for i_s, i_k in T.Parallel(block_S, block_DK):
                    with T.If(G_last_local[0] - G[bb, bs * block_S + i_s, bh] <= 0):
                        with T.Then():
                            dk_fragment[i_s, i_k] = dk_fragment[
                                i_s, i_k] * T.exp(G_last_local[0] - G[bb, bs * block_S + i_s, bh])
                        with T.Else():
                ...
                for i_s1, i_s2 in T.Parallel(block_S, block_S):
                    with T.If(i_s1 >= i_s2 and
                        ...
                        with T.Then():
                            ds_fragment[i_s1, i_s2] = ds_fragment[i_s1, i_s2] * T.exp(
                                G[bb, bs * block_S + i_s1, bh] - G[bb, bs * block_S + i_s2, bh]
                            ) * scale
                        with T.Else():
...
def run_test(
    ...
    kernel = tilelang_chunk_o_bwd_dqkwg(
        ...
        chunk_size, scale, use_g, use_dw, block_DK, block_DV, threads, num_stages
    )
