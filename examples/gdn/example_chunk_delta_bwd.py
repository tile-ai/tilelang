...
def torch_chunk_gated_delta_rule_bwd_dhu(
    ...
    for i_s in range(BS - 1, -1, -1):
        dh[:, i_s, :, :, :] = dh_tmp
        dv_tmp = torch.matmul(
            K[:, i_s * block_S:(i_s + 1) * block_S, :, :].permute(0, 2, 1, 3), dh_tmp.to(K.dtype)
        ).permute(0, 2, 1, 3)
        if use_g:
            for i_bh in range(B * H):
                ...
                for i_s2 in range(block_S):
                    if G[i_b, i_s * block_S + block_S - 1, i_h] - G[i_b, i_s * block_S + i_s2,
                                                                    i_h] <= 0:
                        dv_tmp[i_b, i_s2, i_h, :] *= torch.exp(
                            G[i_b, i_s * block_S + block_S - 1, i_h] -
...
def run_test(
    ...
    kernel = tilelang_chunk_gated_delta_rule_bwd_dhu(
        ...
        chunk_size, scale, use_g, use_initial_state, use_final_state_gradient, block_DV, threads,
        num_stages
    )
