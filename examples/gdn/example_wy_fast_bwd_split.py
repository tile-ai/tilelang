...
def tilelang_wy_fast_bwd(
    ...
    A: T.Tensor[[B, S, H, chunk_size], input_dtype]
    dw: T.Tensor[[B, S, H, DK], input_dtype]
    du: T.Tensor[[B, S, H, DV], input_dtype]

    BS = block_S = chunk_size

    dA = T.empty([B, S, H, BS], input_dtype)
    dk = T.empty([B, S, H, DK], output_dtype)
    dv = T.empty([B, S, H, DV], output_dtype)
    dbeta = T.empty([B, S, H], output_dtype)
    dg = T.empty([B, S, H], gate_dtype)
...
def tilelang_wy_fast_bwd_split(
    ...
    dw: T.Tensor[[B, S, H, DK], input_dtype]
    du: T.Tensor[[B, S, H, DV], input_dtype]
    dA: T.Tensor[[B, S, H, chunk_size], input_dtype]
    dk: T.Tensor[[B, S, H, DK], output_dtype]
    dv: T.Tensor[[B, S, H, DV], output_dtype]
    dbeta_k: T.Tensor[[B, S, H], output_dtype]
    dg_A_positive: T.Tensor[[B, S, H, chunk_size], gate_dtype]
    dg_A_negative: T.Tensor[[B, S, H, chunk_size], gate_dtype]
...
def run_test(
    ...
    dA_tilelang = torch.empty(B, S, H, chunk_size, dtype=getattr(torch, input_dtype)).cuda()
    dbeta_tilelang_k = torch.empty(B, S, H, dtype=getattr(torch, output_dtype)).cuda()
    dg_tilelang_A_positive = torch.empty(B, S, H, chunk_size, dtype=getattr(torch, gate_dtype)).cuda()
    dg_tilelang_A_negative = torch.empty(B, S, H, chunk_size, dtype=getattr(torch, gate_dtype)).cuda()

    # ref
    dk_ref, dv_ref, dbeta_ref, dg_ref = bwd_prepare_wy_repr(K, V, G, Beta, A, dw, du, cu_seqlens=None)

    # tilelang
    dA_tilelang, dk_tilelang, dv_tilelang, dbeta_tilelang, dg_tilelang = tilelang_wy_fast_bwd(
        K,
        V,
        Beta,
        G,
