...
def test_multi_gemm():
    ...
    This pattern exercises a graph of five matmuls with both serial and
    ...
    torch.manual_seed(0)
    x, w1, w2, w3, w4, w5 = [torch.randn(64, 64, device="cuda", dtype=torch.float16) * 0.25 for _ in range(6)]
    torch._dynamo.reset()
